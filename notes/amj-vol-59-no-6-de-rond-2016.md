---
id: amj-vol-59-no-6-de-rond-2016
title: "Some Things Can Never Be Unseen: The Role of Context in Psychological Injury at War"
authors:
  - "de Rond, M."
  - "Lok, J."
year: 2016
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2015.0681"
volume: 59
issue: 6
pages: "1965-1993"

source: "AMJ/vol-59-no-6"
pdf_path: "library/AMJ/vol-59-no-6/pdfs/Rond 2016 Some Things Can Never Be Unseen The Role of Context in Psychological Injury at War.pdf"
text_path: "library/AMJ/vol-59-no-6/text/Rond 2016 Some Things Can Never Be Unseen The Role of Context in Psychological Injury at War.txt"
ingested_at: "2026-07-05"
extraction_model: "gpt-5.5"
extraction_version: "v3"
augmented_model: "gpt-5.6-sol"
augmented_at: "2026-08-31"

paper_type: "empirical-qualitative"
keywords: ["psychological injury", "institutional context", "war", "senselessness", "futility", "surreality", "existential grounding"]
theory: ["context theory (Johns, 2006)", "institutional theory", "existentialism", "coping and normalization"]
topics: ["employee-wellbeing", "qualitative-methods", "institutional-theory", "organizational-identity"]
unit_of_analysis: "team"
level_of_theory: "meso"
dependent_variable_family: "social"
methods: "Ethnography of a military medical team, combining predeployment training observation, a six-week embedded tour in Camp Bastion, fieldnotes, headnotes, reports, hospital records, morbidity and mortality data, photographs, and interpretive coding of distress themes, context, and coping strategies."
sample:
  industry: "Military medicine / damage control surgery field hospital"
  country: "Afghanistan"
  time_period: "April 2010-August 2011, including an embedded tour from June 14 to July 29, 2011"
  units: "Damage control surgery team members and fieldwork events in Camp Bastion"
  n: "At any one time, around 4-5 general surgeons, 6-7 orthopedic surgeons, 1 plastic surgeon, and 5-6 anesthetists, alongside operating department practitioners, theater nurses, and theater coordinators"

evidence:
  sample_n: "around 4–5 general"
  sample_country: "in Camp Bastion, Afghanistan"
  sample_industry: "military medical team in Helmand"
  sample_time_period: "between April 2010 and August 2011"
  theories_overview: "institutional context as a contributing factor"
  methods_overview: "Ethnographic data were collected over 16 months"
  keywords_source: "senselessness, futility, and surreality"
  hypotheses_source: "Not reported in paper"
  measures_overview: "Both authors coded the fieldnotes"
  findings_overview: "sustained experiences of senselessness, futility, and surreality that are partially rooted in cultural"
---

# Some Things Can Never Be Unseen: The Role of Context in Psychological Injury at War

**Abstract**
Recent wars in Iraq and Afghanistan have reignited debates on how to prevent and manage psychological injury among returning troops. These debates point to the psychological cost of war as a grand challenge whose scale and complexity stretch far beyond the already large and growing number of veterans affected. We use a unique ethnography of a military medical team’s tour of duty in Camp Bastion, Afghanistan, to explore the role of institutional context as a contributing factor to psychological injury from war. We find that exposure to war and its consequences invokes sustained experiences of senselessness, futility, and surreality that are partially rooted in cultural expectations, professional role identity, and organizational protocol, and can threaten people’s existential grounding in this institutional context. We argue that what makes work at war traumatic for some and not others is likely affected by the specific context through which people filter, frame, and cope with their experience. A contextual understanding of psychological injury at war that is based in organizational research can thus form an important part of better addressing this grand challenge.

**Research Question**
The paper asks what role cultural, professional, and organizational contexts play in the experience of psychological distress at war. It examines how institutional context shapes what deployed medical personnel experience as traumatic beyond direct exposure to combat threat.

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- Key contexts / expectations: cultural expectations of innocence and normality, professional role identity as medical caregivers, and organizational military protocol.
- Distressing experience themes: senselessness, futility, and surreality.
- Process / dissonance: contextual expectations clash with lived experience in Camp Bastion, especially around children’s suffering, compromised patient care, and surreal juxtapositions of routine and human injury.
- Coping responses: avoiding, bracketing, humor, recovering the familiar, and reclaiming control.
- Emergent outcome: sustained contextual dissonance threatens existential grounding and makes improvised coping strategies only temporarily effective.

The process account is interpretive rather than variable-causal. The authors argue that institutional context filters and amplifies exposure to war by producing dissonance between what medics understand as meaningful, good, and normal and what they encounter in practice. Because the same professional and organizational contexts also inhibit open processing of distress, medics rely on improvised coping strategies that may temporarily distance them from distress but do not resolve its contextual sources.

**Data & Measures**
One author collected ethnographic data over 16 months, from April 2010 through August 2011, including predeployment training and a six-week deployment with a DCS team at Camp Bastion. Fieldnotes formed the main corpus; additional sources included informants’ reflections, poetry, two post-tour reports, hospital admissions and triage data, data from weekly morbidity and mortality meetings, about 1,000 usable photographs, and the ethnographer’s headnotes. Both authors independently coded fieldnotes for distressing events and coping strategies, compared and reconciled codes, temporally bracketed contextual data, and used interpretive and abductive reasoning to develop the model.

**Key Findings**
The team’s lived experience of work at war was characterized by sustained senselessness, futility, and surreality. Cultural, professional, and organizational contexts produced dissonance between institutionalized expectations and experience on the ground; when unresolved and sustained, this dissonance threatened existential grounding. The medics used five improvised coping strategies—avoiding, bracketing, humor, recovering familiarity, and reclaiming control—but these offered only temporary escape and could amplify distress by increasing the contrast between life at war and life at home.

**Theoretical Contribution**
The paper develops a contextual explanation of psychological distress at war that complements individual-level trauma accounts. It contributes to organizational research by showing how context theory and institutional theory can explain existential threat in organizations: institutional context socializes expectations, produces dissonance when those expectations clash with lived experience, and supplies coping resources that may fail or even amplify distress.

**Practical Implication**
Interventions for psychological injury at war should not assume universal trauma triggers or purely individual coping deficits. The paper suggests that prevention and support need to be tailored to the situated cultural, professional, and organizational contexts through which different military groups experience distress.

**Limitations**
The evidence comes from an ethnographic case of rear-located medical personnel in Camp Bastion, so the mechanisms may differ for frontline soldiers, other national militaries, or nonmedical personnel. The authors also frame some implications for PTSD prevention and treatment as suggestions for future research rather than as tested intervention effects.

**Future Research**
Future research should compare how institutional context shapes psychological distress across different military groups, such as frontline troops, reservists, rear-located medics, and personnel leaving service after deployment. It should also examine whether coping strategies that normalize extreme environments can amplify distress by increasing the contrast between life at war and life at home.

**APA 7th Citation**
de Rond, M., & Lok, J. (2016). Some things can never be unseen: The role of context in psychological injury at war. *Academy of Management Journal*, 59(6), 1965-1993. https://doi.org/10.5465/amj.2015.0681
