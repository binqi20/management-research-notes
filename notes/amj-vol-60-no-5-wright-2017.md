---
id: amj-vol-60-no-5-wright-2017
title: "An Inconvenient Truth: How Organizations Translate Climate Change into Business as Usual"
authors:
  - "Wright, C."
  - "Nyberg, D."
year: 2017
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2015.0718"
volume: 60
issue: 5
pages: "1633-1661"

source: "AMJ/vol-60-no-5"
pdf_path: "library/AMJ/vol-60-no-5/pdfs/Wright 2017 An Inconvenient Truth How Organizations Translate Climate Change into Business as Usual.pdf"
text_path: "library/AMJ/vol-60-no-5/text/Wright 2017 An Inconvenient Truth How Organizations Translate Climate Change into Business as Usual.txt"
ingested_at: "2026-07-03"
extraction_model: "gpt-5.5"
extraction_version: "v3"
augmented_model: "gpt-5.6-sol"
augmented_at: "2026-08-29"

paper_type: "empirical-qualitative"
keywords: ["climate change", "grand challenges", "corporate environmentalism", "translation", "business as usual", "critique"]
theory: ["business and the natural environment literature", "new corporate environmentalism", "translation perspective (Czarniawska & Joerges, 1996; Callon, 1986)", "grand challenges"]
topics: ["sustainability-environment", "climate-policy", "csr-strategy", "qualitative-methods"]
unit_of_analysis: "firm"
level_of_theory: "cross-level"
dependent_variable_family: "environmental"
methods: "Longitudinal, comparative qualitative case study of five major Australian corporations, using interviews, corporate documents, media releases, case histories, timelines, open coding, axial coding, and NVivo matrix coding to build a grounded process model."
sample:
  industry: "Five major Australian corporations from electricity and gas, banking, manufacturing, insurance, and media and communications."
  country: "Australia."
  time_period: "2005-2015."
  units: "Corporate case studies, manager interviews, corporate documents, sustainability reports, policy documents, presentations, and media releases."
  n: "Five case studies; 70 interviews; 1,629 pages of transcript; 377 documents; 1,819 pages of documents."

evidence:
  sample_n: "70 interviews; 1,629 pages of transcript"
  sample_country: "major Australian corporations"
  sample_industry: "from different industries"
  sample_time_period: "10-year period (2005–2015)"
  theories_overview: "process of “translation”"
  methods_overview: "five in-depth case studies"
  keywords_source: "corporate translation of climate change"
  hypotheses_source: "Not reported in paper"
  measures_overview: "Using the qualitative data analysis software NVivo"
  findings_overview: "identify three key stages in the corporate translation of climate change: framing, localizing, and normalizing"
---

# An Inconvenient Truth: How Organizations Translate Climate Change into Business as Usual

**Abstract**
Climate change represents the grandest of challenges facing humanity. In the space of two centuries of industrial development, human civilization has changed the chemistry of the atmosphere and oceans, with devastating consequences. Business organizations are central to this challenge, in that they support the production of escalating greenhouse gas emissions but also offer innovative ways to decarbonize our economies. In this paper, we examine how businesses respond to climate change. Based on five in-depth case studies of major Australian corporations over a 10-year period (2005–2015), we identify three key stages in the corporate translation of climate change: framing, localizing, and normalizing. We develop a grounded model that explains how the revolutionary import of grand challenges is converted into the mundane and comfortable concerns of “business as usual.” We find that critique is the major driver of this process by continuously revealing the tensions between the demands of the grand challenge and business imperatives. Our paper contributes to the literature on business and the natural environment by identifying how and why corporate environmental initiatives deteriorate over time. More specifically, we highlight the policy limitations of a reliance on business and market responses to the climate crisis.

**Research Question**
The paper asks how firms engage over time with competing market and social/environmental demands when translating complex challenges such as climate change into practice. It examines why initially strong corporate climate commitments in proactive firms are narrowed, diluted, or redirected toward business-as-usual concerns.

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- Empirical setting: Five proactive Australian corporations facing climate-change pressure across electricity and gas, banking, manufacturing, insurance, and media and communications.
- Triggering tension: External and internal critique makes climate change salient while exposing conflict between business growth and environmental demands.
- Framing: Senior managers associate climate change with business-compatible meanings such as risk, opportunity, innovation, leadership, and win-win outcomes, while disassociating it from sacrifice, regulation, or threats to growth.
- Localizing: Managers make those framings operational through incorporation, commensuration, and proselytization, creating roles, products, metrics, advocacy, and eco-efficiency practices that temporarily align business and environmental concerns.
- Normalizing: Market evaluation later reasserts shareholder-value tests, leading firms to purify climate initiatives back toward core business or dilute them into broader sustainability language.
- Emergent outcome: The revolutionary implications of climate change are translated into familiar corporate routines and less threatening business-as-usual formulations.

The process is dialectical rather than a variable-based causal test. Critique repeatedly reveals tensions between climate demands and business imperatives: initial social/environmental critique prompts framing, dissonance between framing and practice prompts localization, and later market evaluation prompts normalization. The model explains deterioration over time by showing how corporate climate initiatives are continually judged against dominant market discourses, especially under financial stress, leadership change, and shifting political conditions.

**Data & Measures**
The study compares five major Australian corporations across electricity and gas, banking, manufacturing, insurance, and media and communications. The 2005–2015 case comparison draws on 70 semi-structured interviews (1,629 transcript pages) and 377 documents (1,819 pages); the document collection extends back to 2000 to establish company prehistories. The researchers developed case histories and timelines, open-coded practices, strategies, narratives, and discourses in NVivo into more than 60 primary nodes, used axial coding to form second-order themes, and applied matrix coding queries to compare the emergent stages across cases and time.

**Key Findings**
Across all five cases, corporate climate engagement moved through framing, localizing, and normalizing. Firms first associated climate change with business-compatible meanings such as risk, opportunity, and innovation; then incorporated it into roles, products, metrics, advocacy, and eco-efficiency practices; and ultimately purified initiatives back toward core business or diluted them into broader sustainability agendas. Critique repeatedly exposed tensions between environmental demands and market evaluation, and engagement with climate change steadily diminished and narrowed toward shareholder value and business as usual.

**Theoretical Contribution**
The paper contributes to business and the natural environment research by explaining how corporate environmental initiatives deteriorate over time rather than only how they are initially framed or implemented. It develops a grounded model of grand-challenge translation that links framing, localizing, and normalizing through continuous critique. It also qualifies both optimistic win-win accounts and purely cynical greenwashing accounts by showing that managers may be authentically concerned while still constrained by short-term profitability and shareholder-value pressures.

**Practical Implication**
The paper argues that relying primarily on corporate leadership, market mechanisms, or business innovation is insufficient for climate-change response because corporations tend to translate grand challenges into terms compatible with value creation. For policy and practice debates, this implies a need for regulatory guidance, limits on short-term corporate political influence, and incentives for longer-term planning.

**Limitations**
The authors note that the study uses five organizational case studies of proactive Australian firms, so care is needed in generalizing to firms that ignore, reject, or dismiss climate change. They also note that climate change is politically contentious, which may limit direct transfer of the model to less polarized issues. Because the analysis focuses on internal organizational translation, it is partially blind to broader societal and global processes of climate-change translation and to how the process might differ across political regimes.

**Future Research**
The paper calls for future research on organizational dynamics in businesses that reject or dismiss climate change, particularly as the physical and political ramifications of climate change worsen. It also points to research comparing the model across other social and environmental challenges, less contentious issues, less polarized societies, and different political regimes. Further work could examine the explanatory power of globally dominant market discourses and the societal-level translation processes that shape corporate climate action.

**APA 7th Citation**
Wright, C., & Nyberg, D. (2017). An inconvenient truth: How organizations translate climate change into business as usual. *Academy of Management Journal*, 60(5), 1633-1661. https://doi.org/10.5465/amj.2015.0718
