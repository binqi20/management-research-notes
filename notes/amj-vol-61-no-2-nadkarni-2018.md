---
id: amj-vol-61-no-2-nadkarni-2018
title: "New Ways of Seeing: Radical Theorizing"
authors:
  - "Nadkarni, Sucheta"
  - "Gruber, Marc"
  - "DeCelles, Katy"
  - "Connelly, Brian"
  - "Baer, Markus"
year: 2018
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2018.4002"
volume: 61
issue: 2
pages: "371-377"

source: "AMJ/vol-61-no-2"
pdf_path: "library/AMJ/vol-61-no-2/pdfs/Nadkarni 2018 New Ways of Seeing Radical Theorizing.pdf"
text_path: "library/AMJ/vol-61-no-2/text/Nadkarni 2018 New Ways of Seeing Radical Theorizing.txt"
ingested_at: "2026-07-01"
extraction_model: "gpt-5.5"
extraction_version: "v3"
augmented_model: "gpt-5.6-sol"
augmented_at: "2026-08-24"

paper_type: "editorial"
keywords: ["radical theorizing", "theory development", "scientific progress", "distant search", "innovation portfolio"]
theory: ["philosophy of science", "innovation and organizational learning"]
topics: ["methods-theory", "theory-development"]
unit_of_analysis: "na"
level_of_theory: "na"
dependent_variable_family: "na"
methods: "Editorial guidance essay that draws on philosophy of science, innovation and organizational learning research, and examples of prominent management theorists to explain approaches to radical theorizing and offer practical advice for scholars."
sample:
  industry: "Not reported in paper"
  country: "Not reported in paper"
  time_period: "Not reported in paper"
  units: "na"
  n: "Not reported in paper"

evidence:
  sample_n: "Not reported in paper"
  sample_country: "Not reported in paper"
  sample_industry: "Not reported in paper"
  sample_time_period: "Not reported in paper"
  theories_overview: "research on innovation and organizational"
  methods_overview: "This editorial offers ideas on how scholars might"
  keywords_source: "radical theorizing"
---

# New Ways of Seeing: Radical Theorizing

**Abstract**
Not reported in paper

**Research Question**
The editorial asks how management scholars can engage in radical theorizing that generates new ways of seeing and advances theory beyond incremental extension. It focuses on why radical theorizing matters for scientific progress, what forms it can take, and how researchers can cultivate it in their own work.

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- Core argument: Radical theorizing produces completely new theoretical insights that may substantially depart from existing paradigms and open future research directions.
- Approaches: The editorial identifies inductive theory generation, evocative theoretical boundary spanning, theoretical consensus shifting, and diverse theoretical integration as broad routes to radical theorizing.
- Exemplars: The discussion uses prior management theorists, including Denise Rousseau, Barry Staw, and Ken Smith, to illustrate openness to anomalies, broad knowledge, boundary spanning, and diverse collaboration.
- Practical guidance: Scholars are encouraged to read beyond their immediate domain, embrace disagreement and anomalies, see work through others' eyes, spend time inside organizations, and manage a portfolio that includes radical projects.

The process logic is editorial and developmental rather than causal. Nadkarni and colleagues argue that radical theory emerges when scholars move beyond local search, notice anomalies or inadequacies in existing explanations, borrow or integrate distant ideas carefully, and devote part of their research portfolio to transformational questions rather than only incremental extensions.

**Data & Measures**
Not reported in paper

**Key Findings**
Not reported in paper

**Theoretical Contribution**
The editorial contributes a typology of radical theorizing approaches based on the kind of phenomenon being investigated and the state of existing theory. It also frames radical theorizing as a disciplined route to scientific progress, connecting management theory development to philosophy of science and innovation search.

**Practical Implication**
For management scholars, the editorial implies that producing bolder theory requires intentional exposure to distant knowledge, openness to disagreement, attention to practical anomalies, and portfolio-level allocation of time to radical projects. It also suggests that AMJ authors should ask how their manuscripts challenge, change, or advance theoretical understanding before submission.

**Limitations**
The editorial does not test propositions, evaluate systematic evidence, or provide empirical validation of the proposed typology. Its recommendations are broad guidance for theory development rather than a method with guaranteed outcomes.

**Future Research**
The editorial encourages future management research that pursues radical theorizing through new phenomena, distal theories, consensus-challenging explanations, and integration of competing perspectives. It also points to opportunities for scholars to study anomalies and underexplored organizational problems that existing theories do not yet explain well.

**APA 7th Citation**
Nadkarni, S., Gruber, M., DeCelles, K., Connelly, B., & Baer, M. (2018). New ways of seeing: Radical theorizing. *Academy of Management Journal*, 61(2), 371-377. https://doi.org/10.5465/amj.2018.4002
