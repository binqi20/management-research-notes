---
id: amj-vol-63-no-6-hideg-2020
title: "From the Editors: Publishing Practical and Responsible Research in AMJ"
authors:
  - "Hideg, Ivona"
  - "DeCelles, Katherine A."
  - "Tihanyi, Laszlo"
year: 2020
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2020.4006"
volume: 63
issue: 6
pages: "1681-1686"

source: "AMJ/vol-63-no-6"
pdf_path: "library/AMJ/vol-63-no-6/pdfs/Hideg 2020 From the Editors Publishing Practical and Responsible Research in AMJ.pdf"
text_path: "library/AMJ/vol-63-no-6/text/Hideg 2020 From the Editors Publishing Practical and Responsible Research in AMJ.txt"
ingested_at: "2026-06-28"
extraction_model: "gpt-5.5"
extraction_version: "v3"
augmented_model: "gpt-5.6-sol"
augmented_at: "2026-08-01"

paper_type: "editorial"
keywords: ["practical implications", "responsible research", "methodological rigor", "counterintuitive findings", "open science", "diversity of scholars"]
theory: ["Not reported in paper"]
topics: ["business-ethics", "diversity-equity-inclusion", "corporate-social-responsibility", "methods-theory"]
unit_of_analysis: "na"
level_of_theory: "na"
dependent_variable_family: "na"
methods: "Editorial (From the Editors essay): the authors develop normative editorial guidance on publishing practical and responsible AMJ research, organized around harmful practical implications, rigorous methodology, careful interpretation, and collective field responsibility."
sample:
  industry: "Not reported in paper"
  country: "Not reported in paper"
  time_period: "Not reported in paper"
  units: "Not reported in paper"
  n: "Not reported in paper"

evidence:
  sample_n: "Not reported in paper"
  sample_country: "Not reported in paper"
  sample_industry: "Not reported in paper"
  sample_time_period: "Not reported in paper"
  theories_overview: "Not reported in paper"
  methods_overview: "In this editorial, we discuss how to undertake more"
  keywords_source: "responsible research by highlighting"
---

# From the Editors: Publishing Practical and Responsible Research in AMJ

**Abstract**
Not reported in paper

**Research Question**
How should AMJ authors, reviewers, editors, and readers produce and evaluate management research so that its practical implications are rigorous, ethical, and socially responsible rather than harmful or easily misused?

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- Core argument: Management research should help make organizations and society better, so practical implications need to be treated as an essential part of the manuscript rather than as a final technical hurdle.
- Harm-risk focus: Research on sensitive topics can unintentionally reinforce victim blaming, stereotypes, insensitive practices, or harmful organizational decisions if its implications are shallow, sensationalized, or ethically underdeveloped.
- Methodological responsibility: Counterintuitive and controversial findings require especially strong empirical evidence, transparency, replication, preregistration where appropriate, and caution against HARKing, p-hacking, false positives, small effects, and unsupported causal language.
- Collective responsibility: Authors, reviewers, editors, mentors, and audience members should scrutinize methodology, interpretation, and practical implications, while broadening participation by diverse scholars and viewpoints.
- Field-development practices: The editorial recommends presenting work to diverse and interdisciplinary audiences and integrating responsible-management-research discussions into doctoral seminars.

The process logic is normative rather than causal: practical responsibility begins when researchers formulate questions, continues through methodological design and interpretation, and extends into review, publication, and wider communication. Hideg, DeCelles, and Tihanyi argue that sensitive or counterintuitive findings can create social harm when methodological limits and ethical implications are not made explicit. They therefore call for a field-wide practice of coupling theoretical contribution with credible evidence, careful language, and attention to the consequences of research for organizational members and external stakeholders.

**Data & Measures**
Not reported in paper

**Key Findings**
Not reported in paper

**Theoretical Contribution**
The essay is not a theory-building article; it contributes editorial guidance about how AMJ should understand the relation between theoretical contribution, practical implication, and responsible scholarship. It reframes practical implications as ethically consequential claims that must be supported by rigorous methods and careful interpretation, especially when research concerns vulnerable groups, sensitive workplace issues, CSR, discrimination, corruption, harassment, or other socially consequential topics.

**Practical Implication**
Authors should consider potential misuse and unintended consequences from the beginning of the research process, avoid sensationalized framing, and spell out implications concretely and comprehensively. They should also use stronger methodological safeguards for controversial or counterintuitive findings, report results transparently, avoid causal language when causality is not warranted, and present findings to diverse or interdisciplinary audiences before submission. Reviewers and editors should scrutinize practical implications, replicability, transparency, and interpretations of sensitive findings more actively.

**Limitations**
Not reported in paper

**Future Research**
The editorial encourages research that seeks genuine solutions to organizational and societal challenges while being mindful of ethical and social consequences. It also calls for more research done with, rather than merely on, marginalized groups, more attention to developing economies and nontraditional professions, and more doctoral training on responsible and practical management research.

**APA 7th Citation**
Hideg, I., DeCelles, K. A., & Tihanyi, L. (2020). From the editors: Publishing practical and responsible research in AMJ. *Academy of Management Journal*, 63(6), 1681-1686. https://doi.org/10.5465/amj.2020.4006
