---
id: amj-vol-65-no-2-umphress-2022
title: "From the Editors—Insights on How We Try to Show Empathy, Respect, and Inclusion in AMJ"
authors:
  - "Umphress, Elizabeth E."
  - "Rink, Floor"
  - "Muir (Zapata), Cindy P."
  - "Hideg, Ivona"
year: 2022
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2022.4002"
volume: 65
issue: 2
pages: "363-370"

source: "AMJ/vol-65-no-2"
pdf_path: "library/AMJ/vol-65-no-2/pdfs/Umphress 2022 From the Editors—Insights on How We Try to Show Empathy, Respect, and Inclusion in AMJ.pdf"
text_path: "library/AMJ/vol-65-no-2/text/Umphress 2022 From the Editors—Insights on How We Try to Show Empathy, Respect, and Inclusion in AMJ.txt"
ingested_at: "2026-06-23"
extraction_model: "claude-opus-4-8"
extraction_version: "v3"
augmented_model: "claude-opus-5"
augmented_at: "2026-07-30"

paper_type: "editorial"
keywords: ["empathy", "respect", "inclusion", "belongingness", "uniqueness", "peer review", "sample preference", "language barriers"]
theory: ["Not reported in paper"]
topics: ["diversity-equity-inclusion", "dei-initiatives", "inclusive-leadership", "management-education"]
unit_of_analysis: "na"
level_of_theory: "na"
dependent_variable_family: "na"
methods: "Editorial (From the Editors essay): the AMJ editorial team articulates practices and considerations for showing empathy, respect, and inclusion across reviewer, associate-editor, and author roles."
sample:
  industry: "Not reported in paper"
  country: "Not reported in paper"
  time_period: "Not reported in paper"
  units: "Not reported in paper"
  n: "Not reported in paper"

# Mandatory evidence anchors (v2 — Layer 1 faithfulness audit).
evidence:
  sample_n: "Not reported in paper"
  sample_country: "Not reported in paper"
  sample_industry: "Not reported in paper"
  sample_time_period: "Not reported in paper"
  theories_overview: "Empathy, respect, and inclusion are important to"
  methods_overview: "sider empathy, respect, and inclusion in our roles as"
  keywords_source: "we first outline how empathy and respect"
---

# From the Editors—Insights on How We Try to Show Empathy, Respect, and Inclusion in AMJ

**Abstract**
Not reported in paper

**Research Question**
How can the AMJ community—reviewers, associate editors, and authors—demonstrate empathy, respect, and inclusion throughout the manuscript review process in order to maintain and develop an open, inclusive academic community, especially in the wake of disruptive worldwide events?

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- Core argument: Feeling included rests on satisfying both belongingness and uniqueness; treating colleagues with empathy and respect during review interactions helps establish an inclusive environment and can enhance the inclusion of ideas and scholars.
- Key constructs: Empathy (other-oriented feelings congruent with another's perceived welfare); respect (regard for the feelings, wishes, and unique qualities of others); inclusion (balancing belongingness and uniqueness).
- Reviewer considerations: Preventing positive cultural sample preference (e.g., reflexive U.S./Western-sample demands); overcoming language barriers for non-native English speakers; respecting differences in research norms, and monetary and temporal resources; being open to ideas from the review team.
- Associate-editor considerations: Assembling diverse, well-equipped review teams; communicating editorial decisions and "high-risk" revisions with empathy and respect; minimizing revision rounds; clarifying what a decision means for the specific journal.
- Author considerations: Respecting human-research-participant rights and institutional review processes; referring to prior scholarship respectfully; granting reviewers and editors the benefit of the doubt.

The editors argue that small, everyday acts of empathy and respect—not only large-scale structural change—can enhance inclusion within the scholarly community. Drawing on the belongingness/uniqueness account of inclusion, they reason that socially supported and respected scholars reciprocate by treating others with empathy, which builds trust, affirms unique strengths, and creates a more open academic community. They apply this logic concretely to the reviewer, associate-editor, and author roles, surfacing recurring inclusion tensions (sample bias, language, differential resources, decision framing) and recommending empathetic, developmental practices to mitigate them.

**Data & Measures**
Not reported in paper

**Key Findings**
Not reported in paper

**Theoretical Contribution**
The essay is not a theory-building piece; it stakes out an editorial position for AMJ, translating research on belongingness, uniqueness, empathy, respect, and organizational justice into a set of inclusion-oriented norms for the peer-review process. Its contribution is normative and practical: it makes visible the often-implicit ways empathy, respect, and inclusion (or their absence) operate across review roles and frames small interpersonal acts as a complement to larger structural reform in building an inclusive scholarly community.

**Practical Implication**
Reviewers are urged to avoid reflexively demanding U.S./Western replications or devaluing research from other cultural contexts, to focus on theory and empirics rather than minor grammar of non-native English authors, to be sensitive to authors' monetary and temporal resource constraints, and to write developmental, respectful reviews. Associate editors are urged to assemble diverse review teams, explain decisions and "high-risk" revisions clearly, and minimize revision rounds. Authors are urged to protect participants, cite prior work respectfully, and extend reviewers the benefit of the doubt.

**Limitations**
Not reported in paper

**Future Research**
The editors hope these reflections—together with prior and forthcoming discussions of inclusion in the publication process—will prompt continued conversation about demonstrating empathy, respect, and inclusion toward fellow scholars, and will help expand access to authors and reviewers and address research questions that remain less examined in the literature.

**APA 7th Citation**
Umphress, E. E., Rink, F., Muir (Zapata), C. P., & Hideg, I. (2022). From the editors—Insights on how we try to show empathy, respect, and inclusion in AMJ. *Academy of Management Journal*, 65(2), 363–370. https://doi.org/10.5465/amj.2022.4002
