---
id: amj-vol-66-no-1-gruber-2023
title: "From the Editors—An Innovative Journal during Transformational Times: Embarking on the 23rd Editorial Term"
authors:
  - "Gruber, M."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2023.4001"
volume: 66
issue: 1
pages: "1-8"

source: "AMJ/vol-66-no-1"
pdf_path: "library/AMJ/vol-66-no-1/pdfs/Gruber 2023 From the Editors—An Innovative Journal during Transformational Times Embarking on the 23rd Editorial Term.pdf"
text_path: "library/AMJ/vol-66-no-1/text/Gruber 2023 From the Editors—An Innovative Journal during Transformational Times Embarking on the 23rd Editorial Term.txt"
ingested_at: "2026-05-27"
extraction_model: "claude-opus-4-6"
extraction_version: "v2"

paper_type: "editorial"
keywords: ["transformational times", "editorial agenda", "sustainability", "DEI", "artificial intelligence", "methodological innovation"]
theory: ["Not reported in paper"]
topics: ["methods-theory", "diversity-equity-inclusion", "sustainability-environment"]
unit_of_analysis: "na"
level_of_theory: "na"
dependent_variable_family: "na"
methods: "Editorial essay; no empirical methodology. The new editor-in-chief articulates strategic priorities, describes editorial team composition, and outlines workflow changes for the 23rd editorial term (2023-2025)."
sample:
  industry: "Not reported in paper"
  country: "Not reported in paper"
  time_period: "Not reported in paper"
  units: "Not reported in paper"
  n: "Not reported in paper"

evidence:
  sample_n: "Not reported in paper"
  sample_country: "Not reported in paper"
  sample_industry: "Not reported in paper"
  sample_time_period: "Not reported in paper"
  theories_overview: "Not reported in paper"
  methods_overview: "pose a strategic agenda for this 23rd editorial term"
  keywords_source: "that emphasizes three main goals: (1) encourage"
---

# From the Editors—An Innovative Journal during Transformational Times: Embarking on the 23rd Editorial Term

**Abstract**
Not reported in paper

**Research Question**
Not reported in paper

**Mechanism Process**
- Strategic priority 1: Encourage management research that increases our understanding of transformational times (technological, societal, ecological change).
- Strategic priority 2: Embrace new data sources and key methodological advances at AMJ (including AI, machine learning, natural language processing).
- Strategic priority 3: Experiment with innovative ways of engaging with prospective authors, the Academy, and other key stakeholders (podcasts, video series, social media forums, joint PDWs with sister journals).
- Structural changes: third deputy editor position added (micro, macro, qualitative); newly created panel of Consulting Methodological Experts; annual DEI award; expanded digital outreach.

The editorial lays out a three-part agenda for AMJ's 23rd editorial term (2023-2025) under editor-in-chief Marc Gruber. It frames the contemporary context as transformational — driven by AI, hybrid work, sustainability imperatives, and DEI commitments — and positions AMJ to publish cornerstone studies on these emerging realities while preserving rigor. The piece also describes the editorial team's composition (22 members from eight countries, balanced micro/macro/qualitative, gender-balanced) and the revised manuscript workflow.

**Theoretical Contribution**
The editorial argues that "transformational times" call for management scholars to revisit boundary conditions and causal mechanisms rather than anchor on past theories — citing George (2016) on the field's tendency to default to inherited frameworks. It positions AMJ as a venue where new seminal studies can recast existing theories and, where necessary, develop entirely new ones to explain rapidly evolving organizational realities (AI, sustainability transitions, DEI, hybrid work).

**Practical Implication**
The editorial signals to prospective authors that AMJ welcomes submissions addressing transformational phenomena (AI, sustainability, DEI, new work arrangements) and is open to novel data sources, less-common empirical methods, and unconventional theoretical foundations. It also advertises new author-facing supports: a Consulting Methodological Experts panel, monthly podcasts, video series, joint paper development workshops, and a turnaround target of 60 days or less for first decisions.

**Limitations**
Not reported in paper

**Future Research**
The editorial encourages submissions that recast existing theories to fit transformational conditions or develop entirely new theories where current frameworks lack explanatory power. It flags AI-enabled "instant research paradigms," sustainability transitions, DEI scholarship, and the changing nature of work as priority areas, and announces a forthcoming Special Research Forum call on transformational times.

**APA 7th Citation**
Gruber, M. 2023. From the editors—An innovative journal during transformational times: Embarking on the 23rd editorial term. *Academy of Management Journal*, 66(1), 1-8. https://doi.org/10.5465/amj.2023.4001
