---
id: amj-vol-66-no-6-couture-2023
title: "Triggers, Traps, and Disconnect: How Governance Obstacles Hinder Progress on Grand Challenges"
authors:
  - "Couture, F."
  - "Jarzabkowski, P."
  - "Lê, J. K."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2020.1716"
volume: 66
issue: 6
pages: "1651-1680"

source: "AMJ/vol-66-no-6"
pdf_path: "library/AMJ/vol-66-no-6/pdfs/Couture 2023 Triggers, Traps, and Disconnect How Governance Obstacles Hinder Progress on Grand Challenges.pdf"
text_path: "library/AMJ/vol-66-no-6/text/Couture 2023 Triggers, Traps, and Disconnect How Governance Obstacles Hinder Progress on Grand Challenges.txt"
ingested_at: "2026-05-16"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-qualitative"
keywords: ["multistakeholder governance", "grand challenges", "governance obstacles", "governance traps", "path dependence", "collective response", "broadening practices", "containing practices", "water health"]
theory: ["multistakeholder governance (Dentoni et al., 2018; Provan & Kenis, 2008)", "path dependence theory (Sydow, Schreyögg, & Koch, 2009, 2020)", "grand challenges (Ferraro, Etzion, & Gehman, 2015; George et al., 2016)"]
topics: ["sustainability-environment", "stakeholder-engagement", "practice-theory", "grand-challenges"]
unit_of_analysis: "organization"
level_of_theory: "meso"
dependent_variable_family: "environmental"
methods: "In-depth interpretive longitudinal case study combining ethnographic observation, semi-structured interviews, and archival documents; ~500 hours of observation, 30 interviews, and extensive documentary material analyzed in three iterative steps (open coding, axial coding, theoretical abstraction) to develop a process model."
sample:
  industry: "Multistakeholder water-health partnership spanning nonprofit, mining, agriculture, government (local, state, federal), academic, and consultancy organizations"
  country: "Australia (Great Barrier Reef region)"
  time_period: "2008-2018 (historical context 2008-2017; real-time fieldwork in 2018)"
  units: "One multistakeholder initiative (WaterHealthOrg) and its 25 partner organizations"
  n: "1 case study; 30 semi-structured interviews; ~500 hours of observation; multiple archival documents"

# Mandatory evidence anchors (v2 — Layer 1 faithfulness audit).
evidence:
  sample_n: "ducted 30 semi-structured interviews with partners,"
  sample_country: "address degrading water health in Australia’s critical Great Barrier Reef region"
  sample_industry: "25 partner organizations, including nonprofit, min"
  sample_time_period: "We collected over 500 hours of data"
  theories_overview: "In this paper, we adopt a multistakeholder governance perspective to study how people"
  methods_overview: "Our interpretive analysis unfolded over three"
  keywords_source: "how efforts to manage multistakeholder governance obstacles can generate governance traps that shape participants’ ability to collectively respond"
---

# Triggers, Traps, and Disconnect: How Governance Obstacles Hinder Progress on Grand Challenges

**Abstract**
In this paper, we adopt a multistakeholder governance perspective to study how people collectively respond to a grand challenge. Specifically, we show how working through governance obstacles—that is, coordinating and collaborating challenges arising from a multistakeholder governance approach to responding to grand challenges—can erode actors’ ability to mitigate these wicked problems. We illustrate this process through an in-depth case study of WaterHealthOrg, a multistakeholder initiative established to address degrading water health in Australia’s critical Great Barrier Reef region. Our findings reveal how, in an effort to avoid group paralysis or dissolution, actors employ specific practices to address governance obstacles. By doing so, actors set off a cumulative self-reinforcing process, driving them to consolidate rather than critically reflect on and adapt their collective response. Drawing on these insights, we develop a conceptual process model of how efforts to manage multistakeholder governance obstacles can generate governance traps that shape participants’ ability to collectively respond and, ultimately, mitigate grand challenges.

**Research Question**
How are multistakeholder governance obstacles addressed in practice? What are the implications of this for the collective’s ability to mitigate grand challenges?

**Mechanism Process**
- Triggering conditions: Socioenvironmental triggers (e.g., contamination events, fish deaths, coral bleaching, regulatory debates) that shift the salience of the grand challenge and surface interpretive, structural, and relational governance obstacles.
- Key practices: Broadening practices (stretching the scope by including new activities to incorporate shifting interpretations) and Containing practices (counteracting broadening by limiting scope and keeping focus on agreed activities).
- Process / phases: Three sequential cycles — Developing a collective response (Cycle 1), Protecting a collective response (Cycle 2), Leveraging a collective response (Cycle 3) — each generating positive feedback that narrows the group’s remit.
- Emergent outcomes: Governance traps (D1–Dn) — cumulative lock-in dynamics whereby participants struggle to generate knowledge, develop activities, or deviate from the established response path.
- Boundary conditions: Tool-centric collective responses (e.g., the report card) and tightly coupled multistakeholder initiatives in shifting, evaluative grand challenges.

The process model explains how repeated cycles of trigger → obstacle → broadening/containing practice → narrowed remit produce a cumulative, self-reinforcing path-dependent dynamic. As actors become increasingly skilled at taming governance obstacles to keep the group cohesive, they progressively decouple the collective response from the grand challenge it was intended to mitigate — even external shocks that ought to break the path end up reinforcing it.

**Theoretical Contribution**
The paper introduces the concept of governance traps to explain why multistakeholder initiatives addressing grand challenges so often produce unintended consequences, and theorizes the cumulative, self-reinforcing nature of these traps by connecting them to path-dependence theory. It extends path-dependence work by showing that exogenous shocks, rather than breaking a response path, can reinforce it in interorganizational settings, and nuances optimistic accounts of multistakeholder governance as a panacea for wicked problems.

**Practical Implication**
For policymakers, communities affected by grand challenges, and organizational theorists designing interventions, the study highlights that consolidating a multistakeholder collective response — even successfully — risks decoupling the response from the underlying problem. Practices such as exchanging knowledge with external stakeholders, encouraging activities that deviate from the established path, and critically examining whether the response still fits the evolving challenge may help avoid governance traps.

**Limitations**
The study is bounded by its single-case design focused on one grand challenge (degrading water health in the Great Barrier Reef) and one tightly affiliated multistakeholder initiative whose collective response revolved around a specific commensuration tool (the report card). The cyclical, path-dependent dynamics observed may unfold differently in more loosely affiliated collectives, social movements, online platforms, or initiatives with different membership compositions.

**Future Research**
Future research should examine whether the disconnect documented here also emerges in collective responses to other grand challenges (e.g., the COVID-19 pandemic, catastrophic disasters, climate-related field-configuring events), study longer-term implications of participant selection for multistakeholder initiatives, and test whether commensuration strategies, collective tools, and information practices enable or hinder progress on grand challenges. Comparative work on more loosely affiliated collectives could clarify boundary conditions for the self-reinforcing dynamics.

**APA 7th Citation**
Couture, F., Jarzabkowski, P., & Lê, J. K. (2023). Triggers, traps, and disconnect: How governance obstacles hinder progress on grand challenges. *Academy of Management Journal*, 66(6), 1651-1680. https://doi.org/10.5465/amj.2020.1716
