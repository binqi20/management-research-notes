---
id: amj-vol-66-no-6-shulist-2023
title: "“Can I Sell You Avocadoes and Talk to You About Contraception?” “Well, It Depends Which Comes First”: Anchor Roles and Asymmetric Boundaries"
authors:
  - "Shulist, P."
  - "Rivera-Santos, M."
  - "Kistruck, G. M."
  - "Nguni, W."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2020.1821"
volume: 66
issue: 6
pages: "1768-1802"

source: "AMJ/vol-66-no-6"
pdf_path: "library/AMJ/vol-66-no-6/pdfs/Shulist 2023 “Can I Sell You Avocadoes and Talk to You About Contraception” “Well, It Depends Which Comes First” Anchor Roles and Asymmetric Boundaries.pdf"
text_path: "library/AMJ/vol-66-no-6/text/Shulist 2023 “Can I Sell You Avocadoes and Talk to You About Contraception” “Well, It Depends Which Comes First” Anchor Roles and Asymmetric Boundaries.txt"
ingested_at: "2026-05-16"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-qualitative"
keywords: ["role theory", "interrole boundaries", "anchor role", "asymmetric boundaries", "role contrasts", "work-community roles", "Global South", "multirole interactions", "peer education agents", "Tanzania"]
theory: ["role theory (Biddle, 1979, 1986; Katz & Kahn, 1978)", "boundary theory (Ashforth, Kreiner, & Fugate, 2000; Nippert-Eng, 1996)", "work-family/work-work boundary literature (Allen, Cho, & Meier, 2014; Dumas & Sanchez-Burks, 2015)"]
topics: ["role-theory", "organizational-behavior-hrm", "base-of-pyramid"]
unit_of_analysis: "individual"
level_of_theory: "micro"
dependent_variable_family: "social"
methods: "Inductive qualitative grounded-theory study using 53 one-on-one interviews and 10 focus groups across four waves (2016-2021) with 73 Tanzanian peer education agents; supplemented by clinic-staff and HealthOrg interviews, on-site observations, and 29 internal documents. Coded for boundary permeability and flexibility on 216 interrole-boundary instances using a Gioia-style first-order/second-order/aggregate-dimension structure with axial coding."
sample:
  industry: "Self-employed petty trading and subsistence farming combined with a volunteer community role as peer education agents (PEAs) providing family planning counseling for an INGO (HealthOrg)"
  country: "Tanzania (Dar es Salaam, Mara, Mbeya, Morogoro, Mwanza, Njombe, and other regions)"
  time_period: "2016 to 2021 (four waves of data collection)"
  units: "Individual Tanzanian peer education agents who simultaneously held a self-employment work role and a family-planning community role"
  n: "73 PEA volunteers (53 one-on-one interviews of 42 participants; 10 focus groups with 40 participants; 9 overlapping); 216 coded interrole-boundary occurrences for 24 participants"

# Mandatory evidence anchors (v2 — Layer 1 faithfulness audit).
evidence:
  sample_n: "Our core data are interviews of 73 PEA volunteers"
  sample_country: "Tanzania is a low-income East African country"
  sample_industry: "self-employed work role and a family planning counselor community role"
  sample_time_period: "across four waves of data collection from 2016 to"
  theories_overview: "literature on role theory and boundary-setting"
  methods_overview: "53 interviews and 10 focus groups"
  keywords_source: "Role theory generally predicts that when the demands and norms"
---

# “Can I Sell You Avocadoes and Talk to You About Contraception?” “Well, It Depends Which Comes First”: Anchor Roles and Asymmetric Boundaries

**Abstract**
Role theory generally predicts that when the demands and norms of two roles are highly contrasted, individuals will construct a strong boundary to separate the roles. However, such predictions are grounded primarily in the Global North, emphasizing role pairings including “work–family” and hybrid “work–work.” Comparatively, the Global South is characterized by a lack of public services that creates a highly contrasted, highly salient, and yet understudied role pairing—“work–community.” Additionally, the socioeconomic features of the Global South (e.g., dense and overlapping community networks, financial poverty) call into question whether existing predictions surrounding boundary strength are likely to hold. We conducted a qualitative study of 73 Tanzanian participants who had both a self-employed work role and a family planning counselor community role. We found that highly contrasted roles can be simultaneously perceived as both incompatible and compatible. Specifically, the boundaries we observed were neither uniformly strong nor weak, but rather of asymmetric strength: strong when a social interaction was anchored in the community role, but weak when anchored in the work role. The specific role contrasts we identify, and the importance of role anchoring we observe, have important implications for role theory and boundary-setting more broadly.

**Research Question**
What type of interrole boundary do individuals in the Global South construct between contrasting work and community roles, given that role contrasts predict strong boundaries while the dense networks and poverty of Global South contexts predict weak ones?

**Mechanism Process**
- Key constructs / tensions: Role contrasts (script leeway, knowledge intensity, other- vs. self-interested orientation); boundary strength (permeability and flexibility); the novel concept of "anchor role" — the role that initially sets the stage for a specific social interaction.
- Triggering conditions: A multirole interaction is triggered when role holders have both the ability (dense, overlapping social networks; co-located work and community roles) and the motivation (poverty, time scarcity, care for the community role) to bring two roles together with the same person.
- Practices: When anchored in the work (self-employment) role at one's place of business, role holders enacted permeable, flexible boundaries — freely surfacing the PEA role during work interactions. When anchored in the PEA role (e.g., at the clinic or during community education), role holders enacted strong, narrow boundaries and refrained from work-related behaviors.
- Emergent outcomes: Asymmetric (in)compatibility between two highly contrasted roles, producing relationally embedded asymmetric boundaries within the same relationship — strong when anchored in the community role, weak when anchored in the work role.
- Boundary conditions: Asymmetry emerges when (a) role holders have ability and motivation for multirole interactions and (b) role contrasts produce divergent rather than symmetric (in)compatibility between the two roles.

The process logic departs from existing boundary theory by showing that highly contrasted roles need not be uniformly incompatible. A socioeconomic backdrop of poverty, time scarcity, and dense overlapping networks creates a constant integrative pressure to combine roles, but whether that pressure yields a weak boundary depends on the anchor role: when the anchor role's expectations are relatively simple and self-interested (work), bringing in the other-interested community role creates limited expectation violation; when the anchor role is highly scripted, knowledge-intensive, and other-interested (the PEA role), bringing in work-related behaviors triggers norm violations, confusion, and doubt, leading to a strong boundary.

**Theoretical Contribution**
The paper contributes to role theory and boundary-setting by (1) showing that role contrasts do not always render two roles mutually incompatible — contrasts can produce *asymmetric* (in)compatibility; (2) introducing the construct of the "anchor role" and showing that it has a direct, substantial influence on whether an interaction stays single-role or becomes multirole; and (3) theorizing that relationally embedded asymmetric boundaries are not unique to the Global South but extend to work-family (especially family business), work-work (e.g., nurse-manager), and friendship-coworker pairings whenever ability, motivation, and divergent role contrasts coexist.

**Practical Implication**
The findings highlight that combining community and work roles imposes substantial tensions on individuals living in poverty that international NGOs often overlook, because INGOs focus on their donor agreements and ignore that time spent on volunteer roles directly subtracts from livelihood income. Designers of community-health and similar volunteer programs should seek positive spillovers between the community and work roles (e.g., selecting volunteers whose work has some alignment with the community role) and avoid scaling community programs without acknowledging interrole tensions.

**Limitations**
Connections to participants were made through the partner INGO HealthOrg, which may have introduced selection biases despite efforts to diversify by gender, location, and experience. The three observed role contrasts all worked to constrain the PEA role rather than the self-employment role, leaving open the possibility that in other pairings contrasts could constrain the work role instead. The strength of dyadic relationships was not directly measured and may moderate the magnitude of asymmetry, and the study does not theorize how specific role contrasts differentially impact permeability versus flexibility.

**Future Research**
Future work should investigate the magnitude (not just existence) of asymmetric boundaries, including how strong dyadic relationships might attenuate or amplify them; examine settings with "competing" role contrasts that point to opposing anchor-role asymmetries (e.g., a financial planner volunteering at an animal shelter); explore whether ambiguity about which role anchors an interaction produces distinct boundary dynamics; and theorize how specific role contrasts (e.g., knowledge intensity, professionalism) selectively impact the "when" versus "where" dimensions of boundary flexibility.

**APA 7th Citation**
Shulist, P., Rivera-Santos, M., Kistruck, G. M., & Nguni, W. (2023). “Can I sell you avocadoes and talk to you about contraception?” “Well, it depends which comes first”: Anchor roles and asymmetric boundaries. *Academy of Management Journal*, 66(6), 1768-1802. https://doi.org/10.5465/amj.2020.1821
