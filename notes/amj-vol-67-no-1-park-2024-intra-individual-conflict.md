---
id: amj-vol-67-no-1-park-2024-intra-individual-conflict
title: "Intra-Individual Conflict and Task Performance in a Multiteam Context: Examining the Structural Elements of Conflict Experience"
authors:
  - "Park, Semin"
  - "Luciano, Margaret M."
  - "Mathieu, John E."
  - "Fenters, Virgil W."
year: 2024
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2021.0285"
volume: 67
issue: 1
pages: "33-60"

source: "AMJ/vol-67-no-1"
pdf_path: "library/AMJ/vol-67-no-1/pdfs/Park 2024 Intra-Individual Conflict and Task Performance in a Multiteam Context Examining the Structural Elements of Conflict Experience.pdf"
text_path: "library/AMJ/vol-67-no-1/text/Park 2024 Intra-Individual Conflict and Task Performance in a Multiteam Context Examining the Structural Elements of Conflict Experience.txt"
ingested_at: "2026-05-12"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-quantitative"
keywords:
  - "intra-individual conflict"
  - "conflict experience"
  - "multiteam systems"
  - "structural elements of conflict"
  - "directions of conflict (sending vs. receiving)"
  - "team boundaries (within vs. between team)"
  - "self-regulatory process of attentional focus"
  - "on-task attentional pull"
  - "off-task attentional demands"
  - "task performance"
  - "task engagement"
  - "microfoundations"
theory:
  - "intra-individual conflict cognitive process framework (this paper)"
  - "self-regulatory process of attentional focus (Beal, Weiss, Barros, & MacDermid, 2005; Kahneman, 1973)"
  - "cognitive resource allocation theory (Hirst & Kalmar, 1987; Kanfer & Ackerman, 1989)"
  - "multiteam systems theory (Luciano, DeChurch, & Mathieu, 2018; Mathieu et al., 2017)"
  - "communication / sender-receiver model (Gerbner, 1956; Lunenberg, 2010; Weingart et al., 2015)"
topics:
  - "teams-collaboration"
  - "conflict-resolution"
  - "organizational-behavior-hrm"
unit_of_analysis: "individual"
level_of_theory: "micro"
dependent_variable_family: "social"
methods: "Mixed-methods empirical design with two studies. Study 1: observational behavioral coding of six live-actor mass-casualty response training exercises with 13 paramedic students, yielding 3,036 15-second temporal units cross-nested in 110 patients and 6 scenarios; analyzed with three-level hierarchical cross-classified mixed models (lme4 R package). Study 2: online mixed-factorial scenario-based vignette experiment with 400 Qualtrics-recruited US workers (2 directions x 2 boundaries; sending vs. receiving between-subject, within- vs. between-team within-subject); analyzed with random coefficient models (nlme R package) and bootstrapped mediation."
sample:
  industry: "Study 1: emergency medical services (paramedic mass-casualty response training); Study 2: working adults across mixed industries (e.g., manufacturing, construction, retail)"
  country: "United States"
  time_period: "Study 1 data are from mass-casualty exercise event code 1703 (a single day); Study 2 online experiment (date not explicitly reported)"
  units: "Study 1: 3,036 15-second temporal patient-care units cross-nested in 110 patients, 13 paramedics, and 6 scenarios; Study 2: 800 within-subject observations from 400 individual participants"
  n: "Study 1: 3,036 temporal units (13 paramedics; 110 patients; 6 scenarios); Study 2: 400 participants (800 observations)"

# Mandatory evidence anchors (v2 — Layer 1 faithfulness audit).
evidence:
  sample_n: "(3,036 temporal"
  sample_country: "academy in the United States"
  sample_industry: "live-actor mass-casualty response exercises"
  sample_time_period: "exercise event code 1703"
  theories_overview: "regulatory process of attentional focus (Beal et al.,"
  methods_overview: "random coefficient modeling analysis using the"
  keywords_source: "intra-individual microfoundations of conflict in a multiteam context"
---

# Intra-Individual Conflict and Task Performance in a Multiteam Context: Examining the Structural Elements of Conflict Experience

**Abstract**
In this article, we advance a novel intra-individual conflict cognitive process framework, highlighting two structural elements of conflict experiences: (1) directions (sending vs. receiving) and (2) team boundaries (within team vs. between team). Integrating theorizing on cognitive resource allocation, we explain how and why the effect of individuals' conflict experiences on their task performances hinges upon their self-regulation of attention processes guided by the structural elements of conflict experiences. In Study 1, using data extracted from continuous audio and video streams of paramedics participating in live-actor mass-casualty response exercises, we found that sending within-team conflict and receiving between-team conflict related positively with task performance, whereas receiving within-team conflict related negatively to task performance. In Study 2, we conducted a mixed-design scenario-based experiment to examine the underlying mechanisms that drive such effects. Our findings reveal that on-task attentional pull was an important mechanism for a positive sending within-team conflict–task engagement relationship, whereas off-task attentional demands were driving the negative receiving within-team conflict–task engagement relationship. Collectively, we demonstrate that examining the intra-individual microfoundations of conflict in a multiteam context yields unique insights and enables building new theory on when and why conflict is beneficial or harmful.

**Research Question**
How and why do the structural elements of an individual's conflict experiences—the direction of conflict (sending vs. receiving) and the team boundary in which it occurs (within team vs. between team)—shape that individual's task performance in a multiteam context, and what self-regulatory attentional-focus mechanisms account for these differential effects?

**Mechanism Process**
- IVs: Intra-individual conflict experiences classified along two structural dimensions: (a) direction — sending vs. receiving conflict; (b) team boundary — within-team vs. between-team conflict, yielding four conditions (sending within-team, sending between-team, receiving within-team, receiving between-team).
- DVs: Study 1 — task performance, indexed by averaged technical and interpersonal quality ratings of patient care (1–7 Likert) per 15-second temporal unit. Study 2 — task engagement (composite of point-allocation between focal task and conflict plus two Crawford et al. items).
- Mediators (Study 2): on-task attentional pull (sense of control over the conflict issue's interference with task) and off-task attentional demands (felt pressure to address the conflict issue).
- Moderators: team boundary (within vs. between) is theorized to alter the magnitude of each direction-specific self-regulatory path.
- Controls/covariates (Study 1): scenario time, team time, scenario type, team membership dummies, team size, task difficulty, patient priority, cumulative individual conflict (sending and receiving), and collective intra-team and inter-team conflict.

The dual-path attentional-focus mechanism is as follows. The direction of conflict governs which self-regulatory path is engaged: sending conflict mobilizes on-task attentional pull (a sense of control that re-anchors attention on the focal task), while receiving conflict raises off-task attentional demands (a competing pull to address the conflict issue). Team boundaries then modulate the magnitudes of these paths because in-group conflict is perceived as more legitimate and high-priority — strengthening senders' on-task pull within-team but also amplifying receivers' off-task demands within-team. Consequently, sending within-team and receiving between-team conflict are predicted to benefit task performance, whereas receiving within-team conflict is predicted to harm it.

**Theoretical Contribution**
The paper advances a novel intra-individual conflict cognitive process framework that resolves long-standing mixed findings in the conflict literature by aligning the level of theory, phenomenon, and method at the intra-individual level. By introducing two structural elements of conflict experiences—directions (sending vs. receiving) and boundaries (within team vs. between team)—and showing that they trigger distinct self-regulatory attentional pathways (on-task pull for senders; off-task demands for receivers), the framework explains when and why conflict helps or hurts individual performance and elevates conflict as a substantive concern in multiteam-systems theory.

**Practical Implication**
Managers and team members should "pick their battles": even helpful conflict for the sender (sending within-team) can disrupt teammates' task engagement by raising their off-task attentional demands, so senders should clarify the urgency of the issue to allow receivers to manage their attention. Repeated targeting of a single individual (cumulative receiving) is particularly damaging to performance and should be monitored. In multiteam settings, leaders should distinguish within- and between-team conflict because the same behavior carries different performance consequences across these boundaries.

**Limitations**
The Study 1 sample, although behaviorally rich (3,036 15-second units), was accumulated over the course of a single day with 13 paramedic students in low-diversity, physically separated component teams, limiting generalizability across longer time frames, organizational contexts, and MTS configurations. The studies do not directly compare behavioral conflict with perceptual measures, leaving the link between conflict behaviors and summary judgments unexamined. Study 2's online vignette could not fully reproduce the unique information gains that may explain the positive receiving between-team effect observed in Study 1.

**Future Research**
Future work should extend the sampling window to weeks or years to assess short- and long-term effects of cumulative conflict (e.g., normalization, toxic dyads), and broaden the structural-elements lens to include emotions, conflict tactics, and content-specificity manipulations. Researchers should also examine boundary conditions tied to individual differences (e.g., agreeableness, conscientiousness), team conflict norms, component team distance, task importance, and levels of inter-team interdependence, ideally with designs that compare behavioral and perceptual conflict and that test the framework in MTS configurations with greater geographic, cultural, and functional diversity.

**APA 7th Citation**
Park, S., Luciano, M. M., Mathieu, J. E., & Fenters, V. W. (2024). Intra-individual conflict and task performance in a multiteam context: Examining the structural elements of conflict experience. *Academy of Management Journal*, 67(1), 33–60. https://doi.org/10.5465/amj.2021.0285
