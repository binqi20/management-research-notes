---
id: amj-vol-67-no-3-barnes-2023
title: "Becoming a Right-Hand Partner: How Lower-Power Employees Heedfully Challenge Organizational Leaders"
authors:
  - "Barnes, Liza Y."
  - "Lacerenza, Christina N."
  - "Volpone, Sabrina D."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2021.1390"
volume: 67
issue: 3
pages: "704-736"

source: "AMJ/vol-67-no-3"
pdf_path: "library/AMJ/vol-67-no-3/pdfs/Barnes 2023 Becoming a Right-Hand Partner How Lower-Power Employees Heedfully Challenge Organizational Leaders.pdf"
text_path: "library/AMJ/vol-67-no-3/text/Barnes 2023 Becoming a Right-Hand Partner How Lower-Power Employees Heedfully Challenge Organizational Leaders.txt"
ingested_at: "2026-05-07"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-qualitative"
keywords:
  - "right-hand partner"
  - "heedful challenging"
  - "lower-power employees"
  - "unequal-power relationships"
  - "perspective-broadening"
  - "interpersonal learning"
  - "proxying"
  - "chiefs of staff"
  - "loyalty"
  - "upward influence"
theory:
  - "power in organizations (Magee & Galinsky, 2008; Anderson & Brion, 2014)"
  - "perspective-taking and power (Galinsky, Magee, Inesi, & Gruenfeld, 2006; Galinsky et al., 2014)"
  - "unequal-power relationships (Schaerer, du Plessis, Yap, & Thau, 2018)"
  - "employee voice and dissent-based interacting (Morrison, 2014, 2023)"
  - "upward influence and issue selling (Ashford, Rothbard, Piderit, & Dutton, 1998; DiBenigno, 2020)"
topics:
  - "organizational-behavior-hrm"
  - "leadership-behavior"
  - "governance-leadership"
unit_of_analysis: "dyad"
level_of_theory: "micro"
dependent_variable_family: "social"
methods: "Inductive grounded theory study (Strauss & Corbin, 1998; Gioia, Corley, & Hamilton, 2013) of how lower-power employees emerge as right-hand partners. Four iterative phases of data collection and analysis with 74 individuals (61 chiefs of staff and 13 of their matched leaders), comprising 44 one-on-one interviews with COSs (averaging 45 minutes), 13 phone interviews with leaders (averaging 23 minutes), and three 90-minute focus groups (17 COS participants). Analysis used in vivo open coding (~250 codes), axial coding to second-order analytical themes, and theoretical coding into aggregate dimensions; member checking with two COSs and one leader confirmed the model."
sample:
  industry: "Diverse — corporate organizations across technology, healthcare, education, financial services, manufacturing, nonprofit, philanthropy, government, entertainment, and professional services"
  country: "United States (corporate sector; not explicitly named in text but inferred from author affiliations and Prime Chief of Staff partnership)"
  time_period: "Not reported in paper (interviews conducted across four iterative phases prior to publication; participants were currently or recently working as COSs within the past year)"
  units: "Lower-power employee–leader dyads — primarily chiefs of staff and their executive principals (CEOs, COOs, Presidents, etc.)"
  n: "74 individuals (61 chiefs of staff and 13 of their matched leaders)"

evidence:
  sample_n: "Relying on qualitative data from 74 individuals (61 chiefs of staff and 13 leaders)"
  sample_country: "the role and title have recently"
  sample_industry: "in a corporate COS capacity for"
  sample_time_period: "actively, or had been recently (within the past year), working in a corporate COS capacity"
  theories_overview: "we rely on Magee and Galinsky's (2008)"
  methods_overview: "We employed a grounded theory methodology to"
  keywords_source: "we develop a process of heedful challenging: the process of presenting alternative perspectives to someone in an individualized manner"
---

# Becoming a Right-Hand Partner: How Lower-Power Employees Heedfully Challenge Organizational Leaders

**Abstract**
Powerful leaders need to be challenged and pushed to consider uncontemplated perspectives. Research has indicated that employees in lower-power positions are best poised to challenge leaders, because these individuals better understand others' perspectives and have a different lens on day-to-day organizational issues compared to powerful leaders. While several well-known leaders rely on a lower-power employee (i.e., right-hand partner) to amplify their efforts, it is unclear how one becomes a right-hand partner or what this entails. Relying on qualitative data from 74 individuals (61 chiefs of staff and 13 leaders), we develop a process of heedful challenging: the process of presenting alternative perspectives to someone in an individualized manner. In this process, the lower-power employee utilizes their knowledge of the leader and their understanding of how organizational issues are impacting employees to thoughtfully challenge the leader by presenting differing perspectives and illuminating the implications of their behavior. We articulate how one becomes a right-hand partner through this process, present the defining characteristics and drawbacks of being a right-hand partner, and describe instances when this process goes awry. This work contributes to theory about unequal-power relationships and provides insight into how lower-power employees can broaden leader's perspectives.

**Research Question**
How does a lower-power employee become a right-hand partner and influence a high-power organizational leader?

**Mechanism Process**
- Key constructs: heedful challenging; right-hand partner (a lower-power, strategic partner to a powerful leader who possesses inimitable credibility and an ability to discreetly influence the leader); unwavering loyalty; unequal-power relationships
- Process / phases: Phase 1 — Interpersonal learning (acquiring detailed knowledge about the leader through observing publicly and inquiring privately); Phase 2 — Proxying (serving as a deputy or representative of the leader by extending presence and synthesizing information); Phase 3 — Perspective-broadening (challenging the leader by identifying blind spots and illuminating implications of past or intended behavior, conducted in private settings)
- Relational foundation: continuous displays of loyalty enacted before Phase 1 and reinforced throughout each phase, communicating a lack of ulterior motives and alignment with the leader's agenda
- Defining characteristics of the right-hand partner role: strategic partnership (sparring discussions, synchrony, tensility), inimitable credibility, hidden influence
- Drawbacks: one-sided relationship (focus on amplifying the leader, deprioritizing own career), limited recognition (success defined by the success of others, no tangible output)
- Boundary conditions / breakdowns: process can stall at each phase — Phase 1 fails when leaders refuse to invest time; Phase 2 fails when leaders or colleagues resist the COS as a representative; Phase 3 fails when COSs lack confidence or leaders dismiss lower-power input

The process model proposes that right-hand partner emergence unfolds sequentially: lower-power employees first acquire deep knowledge of the leader's intra- and interpersonal processes through observation and probing inquiry, then leverage that knowledge by proxying for the leader in public — gathering ground-level intelligence others would not share with a powerful figure — and finally use the combined understanding of the leader's blind spots and the organization's perspectives to challenge the leader privately, framing critique as perspective-broadening rather than direct attack. Continuous displays of loyalty signal that the challenge is constructive and aligned with the leader's interests, enabling a "yes person" tendency and ingratiation to be replaced by candid, customized, hard-to-deliver feedback that the leader integrates without feeling threatened.

**Theoretical Contribution**
The paper introduces right-hand partners to management scholarship and articulates a process model of their emergence through heedful challenging that addresses three theoretical gaps. First, it extends theory on unequal-power relationships by showing how lower-power employees can wield genuine upward influence without resorting to ingratiation or self-interested issue selling, defying the prevailing view that asymmetric-power dyads are transactional. Second, it operationalizes Galinsky and colleagues' theorizing on the synergistic effects of power and perspective-taking by demonstrating that what is incompatible within a single individual can be realized within a dyad — with the lower-power employee functioning as the perspective-taking catalyst. Third, it advances the dissent-based interacting literature (employee voice, upward influence, issue selling, person-focused feedback) by specifying when, where, and how to deliver hard-to-deliver content (personal critiques, contrarian views) so that it resonates rather than damages the relationship.

**Practical Implication**
Lower-power employees wishing to voice alternative views to organizational leaders should invest extensive time in learning about the leader, frame dissent through perspective-broadening rather than direct criticism, and deliver such feedback in private to protect the leader's public credibility and signal continued loyalty. Powerful leaders who wish to access diverse perspectives, navigate complexity, or guard against the well-documented overconfidence and perspective-taking deficits of high-power roles should consider hiring a chief of staff and structuring the relationship to permit candid, private challenge. Organizations should recognize and legitimate the largely invisible, behind-the-scenes work of right-hand partners despite the difficulty of measuring its impact on company performance.

**Limitations**
The sampling strategy — asking COSs to introduce their leaders — likely biased the leader sub-sample toward those with positive COS relationships, leaving more dysfunctional dyads underrepresented. The study focuses narrowly on the lived experience of right-hand partners themselves and on the corporate COS context, so it cannot fully characterize how the relationship is experienced or shaped by the leader, by demographic diversity within the dyad, or by other organizational members affected by it. Because power is relative, the authors cannot make claims about the amount of power right-hand partners hold over others, leaving open the possibility of a "dark side" in which a right-hand partner uses inauthentic loyalty to accumulate personal power.

**Future Research**
Future work should examine right-hand partners outside the corporate COS context — including government, military, athletic teams, the arts, ghost writers, and graduate-student–advisor dyads — to test boundary conditions of the heedful challenging process. Additional research could investigate whether having a right-hand partner steers leaders away from unethical behavior, improves operational and firm performance, and reshapes the leader's identity, as well as how demographic homophily (especially gender, given the predominance of women in COS roles versus men in executive roles) shapes right-hand partner emergence. Longitudinal designs could trace how long individuals remain right-hand partners and what propels transitions out of the role, and could probe whether some right-hand partners pursue their own power agendas inauthentically.

**APA 7th Citation**
Barnes, L. Y., Lacerenza, C. N., & Volpone, S. D. (2023). Becoming a right-hand partner: How lower-power employees heedfully challenge organizational leaders. *Academy of Management Journal*, 67(3), 704-736. https://doi.org/10.5465/amj.2021.1390
