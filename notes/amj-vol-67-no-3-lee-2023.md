---
id: amj-vol-67-no-3-lee-2023
title: "External Disruption Following CEO Succession: How Human and Social Capital Changes May Attract Competitive Attacks"
authors:
  - "Lee, E. Y."
  - "Tsai, W."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2022.0448"
volume: 67
issue: 3
pages: "625-647"

source: "AMJ/vol-67-no-3"
pdf_path: "library/AMJ/vol-67-no-3/pdfs/Lee 2023 External Disruption Following CEO Succession How Human and Social Capital Changes May Attract Competitive Attacks.pdf"
text_path: "library/AMJ/vol-67-no-3/text/Lee 2023 External Disruption Following CEO Succession How Human and Social Capital Changes May Attract Competitive Attacks.txt"
ingested_at: "2026-05-07"
extraction_model: "claude-opus-4-6"
extraction_version: "v2"

paper_type: "empirical-quantitative"
keywords:
  - "CEO succession"
  - "human capital"
  - "social capital"
  - "predecessor-successor professional experience differential (PSPED)"
  - "insider-outsider successor"
  - "competitive attacks"
  - "external disruption"
  - "preemption"
  - "board chair retention"
theory:
  - "executive human capital theory (Becker, 1964; Bailey & Helfat, 2003)"
  - "social capital theory (Nahapiet & Ghoshal, 1998; Adler & Kwon, 2002)"
  - "competitive dynamics (Chen & Miller, 2012; Smith, Grimm, & Gannon, 1992)"
  - "CEO succession literature (Berns & Klarner, 2017; Shen & Cannella, 2002; Zhang & Rajagopalan, 2010)"
topics:
  - "governance-leadership"
  - "competitive-strategy"
  - "social-capital-theory"
unit_of_analysis: "firm"
level_of_theory: "meso"
dependent_variable_family: "financial"
methods: "Negative binomial regression with Huber-White robust standard errors and year fixed effects on a panel of 175 unique CEO succession events at S&P 500 firms (2010-2018). PSPED operationalized as Euclidean distance between predecessor and successor on eight binary professional-experience attributes (functional background, military, international, outside-board, founder). Two-stage extended negative binomial models with instrumental variables (ranked PSPED differential within industry; new independent directors) address endogeneity; supplementary Poisson, Heckman two-stage, and matched-sample analyses test robustness. Competitive attacks coded from RavenPack co-mention data verified via Factiva."
sample:
  industry: "S&P 500 firms across multiple industries (multi-industry, identified by four-digit SIC code in Mergent Intellect)"
  country: "United States (S&P 500 index firms)"
  time_period: "2010-2018"
  units: "CEO succession events at focal firms (firm-year observations in the post-succession year)"
  n: "175 unique succession events"

evidence:
  sample_n: "Note: n 5 175."
  sample_country: "listed on the S&P 500 index and included in the"
  sample_industry: "listed on the S&P 500 index and included in the"
  sample_time_period: "In our sample of S&P 500 firms between 2010 and 2018, we find"
  theories_overview: "Drawing from theories of executive human and social capital"
  methods_overview: "employed a negative binomial model, which uses a"
  keywords_source: "competitors are more likely to attack the focal firm when the successor's professional experience differs drastically"
---

# External Disruption Following CEO Succession: How Human and Social Capital Changes May Attract Competitive Attacks

**Abstract**
Competitors may perceive a focal firm's CEO succession as creating an exploitable opportunity as well as a future threat to preempt. Drawing from theories of executive human and social capital, we suggest that anticipation of internal disruption stemming from human and social capital changes in a CEO succession may lead to external disruption for the focal firm. In our sample of S&P 500 firms between 2010 and 2018, we find that competitors are more likely to attack the focal firm when the successor's professional experience differs drastically from that of the predecessor and less likely to do so when the successor is an insider. We also find that competitors' attacks following a succession event can be minimized by limiting changes to only one form of capital (i.e., human or social). Moreover, retaining the predecessor as board chair after the succession could be an effective human capital preservation strategy for reducing competitors' attacks when the difference in professional experience between the predecessor and successor is high. However, retaining the predecessor as board chair may not have the same effect when it comes to social capital. Our research highlights the competitive implications of CEO succession, offering new insight into disruption.

**Research Question**
"How do internal disruptions due to human or social capital changes in the CEO position lead to external disruption by attracting competitors' attacks?" The paper investigates how human and social capital changes accompanying CEO succession motivate rivals to launch competitive attacks against the focal firm, and whether retaining the predecessor as board chair attenuates these external disruptions.

**Mechanism Process**
- IV(s): Predecessor–successor professional experience differential (PSPED, Euclidean distance over eight experiential attributes); insider–outsider successor (binary).
- DV(s): Total number of competitive attacks the focal firm receives from rivals in the post-succession year (count, RavenPack co-mention data).
- Mediators: Implicitly, competitors' anticipations of (a) an exploitable opportunity (the focal firm's adjustment-period vulnerability) and (b) a future threat to preempt (the focal firm becoming a more formidable opponent); not directly measured.
- Moderators: Successor insider–outsider status moderates the PSPED–attacks link; predecessor remaining as board chair moderates the PSPED–attacks link (and was hypothesized but not supported as a moderator of the insider–attacks link).

The mechanism: large PSPED signals strategic discontinuity (different "thought worlds") that creates short-run productivity drag (an exploitable vulnerability) and a longer-run repositioning that threatens rivals (a preemption motive); outsider status signals weak internal social capital, predicting power struggles and infighting (an additional vulnerability). Because human and social capital "mutually influence" one another, change in only one form is less attractive to attackers than change in both. Retaining the predecessor as chair preserves the focal firm's human capital (constraining the successor's strategic discretion) and dampens rivals' incentive to exploit PSPED-driven disruption — but does not similarly buffer social-capital disruption.

**Theoretical Contribution**
The paper develops a theory of *external* disruption following CEO succession, extending a literature that has confined succession's disruptive consequences to internal effects (performance, turnover) and largely ignored competitor responses. It introduces the PSPED as a new construct capturing the magnitude of human-capital change, links corporate governance to competitive dynamics by showing that a governance event drives rival attacks, and reframes the practice of retaining the predecessor as board chair from a discretion-limiting or symbolic gesture into a substantive human-capital preservation strategy that wards off competitive attacks.

**Practical Implication**
Boards seeking organizational change through CEO succession should consider candidates who change only the firm's human or its social capital structure, not both — appointing an outsider who is also vastly different from the predecessor maximizes external disruption. When meaningful disruption is necessary, retaining the predecessor as board chair can dampen competitor attacks following a high-PSPED succession by signaling continuity and constraining the successor's discretion to enact disruptive strategic change.

**Limitations**
The study examines only the competitive landscape, leaving other forms of external disruption (regulators, social movements, activist investors) unexplored. The authors cannot directly observe managers' thought processes about whether a focal succession is perceived as an exploitable opportunity or a future threat to preempt. The PSPED measure places equal weight on different professional-experience attributes, ignoring the substantive content of specific functional transitions (e.g., marketing-to-law versus marketing-to-finance).

**Future Research**
Future studies could (a) extend the disruption framework to other external stakeholders (regulators, social movements, ESG-focused activists) and to industry-wide succession waves; (b) qualitatively or experimentally identify the specific cues that lead competitors to assess a CEO succession as an opportunity or a future threat; (c) investigate preemption more broadly as a competitive behavior, given the scarcity of empirical work; and (d) examine whether PSPED's effects vary by the substantive content of professional-experience changes (e.g., functional area combinations, functional vs. industry experience).

**APA 7th Citation**
Lee, E. Y., & Tsai, W. (2023). External disruption following CEO succession: How human and social capital changes may attract competitive attacks. *Academy of Management Journal*, 67(3), 625-647. https://doi.org/10.5465/amj.2022.0448
