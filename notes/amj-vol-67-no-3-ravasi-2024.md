---
id: amj-vol-67-no-3-ravasi-2024
title: "What Makes Research Collaborations Successful? Advice from AMJ Authors"
authors:
  - "Ravasi, Davide"
  - "Zhu, Jing"
  - "Wan, William"
  - "Dorobantu, Sinziana"
  - "Gruber, Marc"
year: 2024
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2024.4003"
volume: 67
issue: 3
pages: "583-594"

source: "AMJ/vol-67-no-3"
pdf_path: "library/AMJ/vol-67-no-3/pdfs/Ravasi 2024 What Makes Research Collaborations Successful Advice from AMJ Authors.pdf"
text_path: "library/AMJ/vol-67-no-3/text/Ravasi 2024 What Makes Research Collaborations Successful Advice from AMJ Authors.txt"
ingested_at: "2026-05-07"
extraction_model: "claude-opus-4-7"
extraction_version: "v3"
augmented_model: "claude-opus-4-8"
augmented_at: "2026-07-12"

paper_type: "editorial"
keywords: ["research collaboration", "co-authorship", "international collaboration", "AMJ authors", "research teams", "authorship order", "scholarly partnerships", "team science"]
theory: ["research collaboration framework (Katz & Martin, 1997)"]
topics: ["methods-theory", "theory-development"]
unit_of_analysis: "na"
level_of_theory: "na"
dependent_variable_family: "na"
methods: "Editorial based on a survey distributed to a randomly picked set of authors of co-authored papers accepted by AMJ between January 1, 2019 and May 16, 2024 (548 contacted, 60 responded, 29 with international collaboration experience), combined with descriptive analysis of authorship patterns across 364 articles and 1,084 authors."
sample:
  industry: "Not reported in paper"
  country: "Not reported in paper"
  time_period: "Not reported in paper"
  units: "Not reported in paper"
  n: "Not reported in paper"
---

# What Makes Research Collaborations Successful? Advice from AMJ Authors

**Abstract**
FROM THE EDITORS WHAT MAKES RESEARCH COLLABORATIONS SUCCESSFUL? ADVICE FROM AMJ AUTHORS

**Research Question**
What enables collaborative projects published in AMJ to overcome the challenges of research collaboration, and what lessons learned from recent AMJ authors can help prospective co-authors take full advantage of the benefits of joint projects while mitigating their risks?

**Hypotheses / Propositions**
Not reported in paper

**Mechanism Process**
- People matter (more than project or location): Get to know potential partners first; work with people you trust and get along well with; ensure work styles and ethics fit; choose people equally motivated to complete the project; reflect on your own unique value proposition.
- Set clear expectations early on: Clarify expected outcomes, roles and contributions, leadership and authorship order, and working style/timelines/constraints/availabilities.
- Manage the process: Establish clear schedules, milestones, and deliverables; communicate frequently and openly; make the best of available technologies (Zoom, cloud storage, online docs) while preserving in-person meetings for breakthroughs.
- Ensure shared accountability for data collection and analysis: Enable equal access to data; share contextual knowledge; "trust but verify" in light of elevated concerns about data integrity.
- Be mindful of differences and open minded: Be open to alternative viewpoints and approaches; treat collaboration as an opportunity to question and revisit assumptions; avoid ethnocentrism in international teams.
- Manage strategically your portfolio of collaborations: Focus efforts on big ideas; search for synergies among projects; balance senior and same-stage co-authors.

The editorial draws on responses from a survey of 60 AMJ authors (drawn from 1,084 authors of 364 articles accepted between January 2019 and May 2024) to articulate six bundles of practices that distinguish successful collaborations. Descriptive analysis shows 53% of manuscripts resulted from cross-country collaborations and 41% from cross-continent collaborations, with COVID-19 producing a delayed and lasting decline in international collaborations. The advice synthesizes practitioner wisdom rather than testing a causal model.

**Data & Measures**
Not reported in paper

**Key Findings**
Not reported in paper

**Theoretical Contribution**
The editorial offers a practitioner-oriented synthesis of what makes research collaborations successful in management scholarship, organizing AMJ author wisdom into six interrelated practice bundles spanning partner selection, expectation setting, process management, data accountability, openness to differences, and portfolio strategy. It also documents empirical patterns in AMJ co-authorship over 2019–2024, including the disproportionate and persistent impact of COVID-19 on international collaborations despite the rise of remote-work technologies.

**Practical Implication**
The editorial provides concrete, actionable advice to current and prospective AMJ authors — especially junior scholars — on how to choose collaborators, set expectations about authorship order and roles up front, manage remote and in-person interactions, and curate a balanced portfolio of collaborations. Table 1 summarizes the advice as a usable checklist, and the editors highlight the heightened importance of shared data accountability given growing concerns about research integrity.

**Limitations**
Not reported in paper

**Future Research**
Not reported in paper

**APA 7th Citation**
Ravasi, D., Zhu, J., Wan, W., Dorobantu, S., & Gruber, M. (2024). What makes research collaborations successful? Advice from AMJ authors. *Academy of Management Journal*, 67(3), 583-594. https://doi.org/10.5465/amj.2024.4003
