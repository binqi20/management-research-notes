---
id: amj-vol-67-no-3-sabanci-2023
title: "The Outcomes of Cross-Category Career Moves: How Cross-Industry Mobility and Industry Prestige Jointly Impact Executive Compensation"
authors:
  - "Sabanci, H."
  - "Elvira, M. M."
year: 2023
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2020.1423"
volume: 67
issue: 3
pages: "767-797"

source: "AMJ/vol-67-no-3"
pdf_path: "library/AMJ/vol-67-no-3/pdfs/Sabanci 2023 The Outcomes of Cross-Category Career Moves How Cross-Industry Mobility and Industry Prestige Jointly Impact Executive Compensation.pdf"
text_path: "library/AMJ/vol-67-no-3/text/Sabanci 2023 The Outcomes of Cross-Category Career Moves How Cross-Industry Mobility and Industry Prestige Jointly Impact Executive Compensation.txt"
ingested_at: "2026-05-07"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-quantitative"
keywords:
  - "cross-industry mobility"
  - "executive compensation"
  - "industry prestige"
  - "categorical affiliation"
  - "category status"
  - "status leap"
  - "status leak"
  - "voluntary executive mobility"
theory:
  - "categorical imperative / category-spanning theory (Zuckerman, 1999; Hsu, Hannan, & Koçak, 2009)"
  - "status-attainment theory (Bidwell et al., 2015; Rider & Tan, 2015; Podolny, 2005)"
  - "category status / industry prestige (Sharkey, 2014; Delmestri & Greenwood, 2016)"
  - "signaling theory (Spence, 1973)"
topics:
  - "executive-compensation"
  - "ceo-leadership"
  - "labor-economics"
  - "signaling-theory"
unit_of_analysis: "individual"
level_of_theory: "cross-level"
dependent_variable_family: "financial"
methods: "Matched-sample quasi-experimental design using one-to-one (k2k) coarsened exact matching (CEM) on individual-, firm-, and pre-pay covariates; difference-in-differences regressions of log total compensation (TDC1) on industry-changer x post-job-change interaction with three-year pre/post window; moderation by status-leap and status-leak dummies derived from Gallup industry prestige rankings; robustness checks with propensity score matching, full unmatched panel, continuous status-difference measures, executive fixed effects, and industry-relatedness controls (Hoberg-Phillips text-based classification)."
sample:
  industry: "S&P 1500 firms across NAICS 2-digit business sectors (Gallup-rated industries)"
  country: "United States"
  time_period: "Job changes 1999-2014 (compensation observations 1996-2017 for 3-year window; 1994-2019 for 5-year robustness)"
  units: "Voluntary inter-firm executive job changes (CEOs and non-CEOs)"
  n: "922 matched job changers (461 industry-changers, 461 within-industry movers); 6,323 executive-year observations; status moderation subsample of 232 leapers and 229 leakers"

evidence:
  sample_n: "n (matched job-changers) 5 922; n (industry-changers) 5 461; n (within-industry movers) 5 461."
  sample_country: "Americans aged 18 and older across all 50 U.S. states"
  sample_industry: "mobility and compensation patterns of S&P 1500"
  sample_time_period: "executives who changed jobs between 1999 and"
  theories_overview: "Identifying executives’ industry affiliation with categorical membership"
  methods_overview: "one (k2k) coarsened exact matching (CEM) (Iacus,"
  keywords_source: "incorporating the influence of industry prestige on"
---

# The Outcomes of Cross-Category Career Moves: How Cross-Industry Mobility and Industry Prestige Jointly Impact Executive Compensation

**Abstract**
Identifying executives’ industry affiliation with categorical membership, this study examines how moving to a different industry impacts mobility-compensation outcomes. On the demand side, we propose that audience ambiguity and commitment concerns regarding cross-category moves limit the potential compensation of industry-changing executives. On the supply side, we argue that executives might accept smaller monetary rewards in exchange for acquiring experience in a new domain. Since category status also affects audience evaluations of candidates and candidates’ desire to affiliate with a specific social category, we further hypothesize that both demand and supply mechanisms are moderated by status differences between an executive’s origin and destination industries. Our analysis of voluntary mobility and compensation patterns of S&P 1500 executives supports these arguments: industry-changing executives realize lower compensation than closely matched within-industry movers. As expected, the compressing effect of changing industry on compensation is contingent upon status differences across industries. Executives who transition to higher-status industries face more stringent compensation discounts, while those moving to lower-status industries experience similar compensation variation as within-industry movers. Our study advances category and status-attainment research by incorporating the influence of industry prestige on career outcomes of cross-category moves, while casting light on the individual consequences of executive mobility.

**Research Question**
How does a change in industry impact compensation in executives’ voluntary job mobility, and how do status differences between origin and destination industries jointly moderate the demand- and supply-side mechanisms that govern the cross-category compensation discount?

**Mechanism Process**
- IV(s): Industry-changer (NAICS 2-digit cross-industry move = 1) interacted with post-job-change indicator; status-leap (move to higher-status industry) and status-leak (move to lower-status industry) dummies based on Gallup prestige rankings.
- DV(s): Log of total executive compensation (ExecuComp TDC1, winsorized at top/bottom 1%).
- Mediators: None formally tested; conceptual mediating mechanisms are demand-side audience ambiguity and future-commitment concerns, and supply-side valuation of new-domain experience and nonpecuniary status rewards.
- Moderators: Industry-status differential between origin and destination industries (operationalized as status-leap vs. status-leak dummies, with continuous status-difference measures in robustness tests).
- Controls: Executive age, gender, tenure, CEO indicator, prior pay rank, prior compensation, firm ROA, sales, debt, capital expenditures, profit margin, sales growth, board size, board independence, CEO duality, board directorship, network size, and gender/industry diversity; industry-relatedness (Hoberg-Phillips) in alternative-explanation tests.

Demand-side audiences (hiring firms and search intermediaries) discount industry-changers because cross-category moves trigger ambiguity about how an executive’s attributes transfer to a new domain and raise future-commitment concerns about flight risk. Supply-side candidates accept smaller monetary premiums because they ascribe value to acquiring experience in a new industry, expanding networks, and capturing nonpecuniary rewards such as prestige. Industry-status differentials condition both mechanisms: leaks (moves down the status hierarchy) attenuate audience ambiguity/commitment penalties and dampen candidate willingness to accept lower pay, while leaps (moves up the status hierarchy) amplify audience scrutiny and candidate willingness to trade compensation for prestige.

**Theoretical Contribution**
The paper introduces categorical status as a new contingency in the categorical-affiliation–labor-market-outcomes relationship, joining the vertical (status) and horizontal (category) dimensions of social evaluation. It separates industry-changing from generalist/specialist profiles and unpacks both demand- and supply-side mechanisms behind cross-category career discounts. By conceptualizing industry status as a novel reward form that can substitute for monetary compensation, it advances status-attainment research beyond firm-level status comparisons within a single industry and offers an integrative framework linking category dynamics, status hierarchies, and executive mobility outcomes.

**Practical Implication**
Boards, compensation committees, and executive search firms should recognize that cross-industry hires accept systematically lower pay than within-industry movers, but this discount is contingent on relative industry prestige—status leapers will accept the steepest discounts and status leakers little or none. Executives weighing voluntary moves can use industry prestige as a non-pecuniary reward when negotiating compensation packages, while firms in lower-prestige sectors should expect to pay a premium to attract talent from higher-prestige industries.

**Limitations**
Executive mobility is endogenous and lacks an exogenous shock, so causal interpretation requires caution despite the matching, comparative-trends, and rich-controls strategies the authors employ. The design cannot directly observe recruiter and candidate cognitions, so the authors cannot definitively partition the observed compensation effects between demand-side and supply-side mechanisms. The S&P 1500 setting contains very few women and underrepresented racial groups, precluding analysis of how ascriptive characteristics interact with the proposed mechanisms.

**Future Research**
Future work could examine whether the mechanisms differ across audiences and candidates—particularly for women and ethnic minorities, where ascriptive characteristics may shape recruiters’ assessments of agency and ambiguity. Researchers could exploit experimental designs or natural experiments to isolate demand- versus supply-side contributions to the cross-industry discount, extend the framework to more diverse empirical contexts beyond U.S. S&P 1500 firms, and investigate how industry status interacts with other forms of categorical stigma or with occupational status in shaping labor-market outcomes.

**APA 7th Citation**
Sabanci, H., & Elvira, M. M. (2024). The outcomes of cross-category career moves: How cross-industry mobility and industry prestige jointly impact executive compensation. *Academy of Management Journal*, 67(3), 767-797. https://doi.org/10.5465/amj.2020.1423
