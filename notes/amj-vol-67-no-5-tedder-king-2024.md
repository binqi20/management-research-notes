---
id: amj-vol-67-no-5-tedder-king-2024
title: "Fairness Judgments in the Context of Structural Sexism: The Role of Beliefs in Individual and Structural Causes of Success"
authors:
  - "Tedder-King, Alyssa"
  - "Sherf, Elad N."
year: 2024
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2022.0776"
volume: 67
issue: 5
pages: "1234-1272"

source: "AMJ/vol-67-no-5"
pdf_path: "library/AMJ/vol-67-no-5/pdfs/Tedder-King 2024 Fairness Judgments in the Context of Structural Sexism The Role of Beliefs in Individual and Structural Causes of Success.pdf"
text_path: "library/AMJ/vol-67-no-5/text/Tedder-King 2024 Fairness Judgments in the Context of Structural Sexism The Role of Beliefs in Individual and Structural Causes of Success.txt"
ingested_at: "2026-05-05"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-quantitative"
keywords: ["fairness judgments", "structural sexism", "meritocracy", "equity", "beliefs in causes of success", "performance indicators", "diversity equity inclusion", "social justice"]
theory: ["equity theory", "meritocracy", "distributive justice / organizational fairness", "attribution theory"]
topics: ["diversity-equity-inclusion", "gender-in-organizations", "organizational-behavior-hrm", "north-america"]
unit_of_analysis: "individual"
level_of_theory: "micro"
dependent_variable_family: "social"
methods: "Twelve preregistered vignette experiments (online via MTurk, Prolific, Connect; one lab study) using 2x2 factorial designs and a within-person design. Participants assumed the role of a senior manager observing a junior manager's allocation decision (raw performance vs. adjusting for structural sexism). Analyses used OLS regression and multilevel/random-coefficient modeling in R with Johnson-Neyman simple-slopes tests, regression-based path analysis with 5,000 bootstraps in Mplus 8.10 for mediated moderation, and an internal meta-analysis (psychmeta) across studies."
sample:
  industry: "Vignette experiments simulating workplace allocation decisions (bonuses, management training program, employee-of-the-year award, firefighter promotion) with junior-manager allocators"
  country: "United States"
  time_period: "Not reported in paper"
  units: "Adult observers (online and lab participants) judging managerial allocation decisions"
  n: "Across 12 studies, k=11 / N=3,748 in the internal meta-analysis of common-procedure conditions; focal Studies 1-4 had N=330 (within-person), 1,067, 366, and 672"

evidence:
  sample_n: "k 5 11, N 5 3,748, data are from our common procedure conditions"
  sample_country: "were drawn from the United States"
  sample_industry: "an allocation (e.g., employee of the year, bonus, or"
  sample_time_period: "Not reported in paper"
  theories_overview: "Fairness scholars often equate distributive fairness"
  methods_overview: "We conducted 12 studies (10 preregistered) to test"
  keywords_source: "Due to structural sexism, women frequently encounter barriers"
---

# Fairness Judgments in the Context of Structural Sexism: The Role of Beliefs in Individual and Structural Causes of Success

**Abstract**
Due to structural sexism, women frequently encounter barriers that result in lower rankings on performance indicators such as productivity data and appraisals by others. The fairness literature offers unclear predictions about the extent to which observers would judge allocations that fail (vs. attempt) to account for such structural barriers as fair. We suggest that past conceptual discussions and empirical designs did not factor in the causal ambiguity embedded in information about structural barriers and the variance in how individuals interpret such information. Hence, we propose that beliefs in the causes of success, anchored in the endorsement of meritocratic ideals, shape fairness judgments by determining the weight given to performance indicators and structural barriers. Such effects emerge in the presence of information about structural barriers in the context of the decision, a feature that is absent from past studies. We discuss our theory and findings in the context of calls for bridging organizational fairness and social justice research and for a nuanced understanding of people's responses to violations of the meritocratic ideal within organizations.

**Research Question**
When and how do observers' beliefs in the individual versus structural causes of success shape their fairness judgments of managerial allocations that adjust (vs. fail to adjust) raw performance indicators in light of evidence of structural sexism?

**Mechanism Process**
- IV(s): Allocation condition (raw performance = "0" vs. adjusting for structural sexism = "1")
- DV(s): Fairness judgments of the junior manager's decision; manager performance evaluations (e.g., recommended bonus)
- Moderators: Beliefs in individual causes of success ("Rewarding"), beliefs in structural causes of success ("Rigged"); presence vs. absence of contextual information about structural barriers (Hypothesis 3); type of structural barrier (gender-based vs. non-gender-based / task difficulty; subjective interpersonal-bias vs. objective equipment-based)
- Mediator: Fairness judgments mediating the effect of allocation condition on performance evaluations
- Controls: Participant gender; luck (random) beliefs

Drawing on equity theory and the meritocratic ideal, the authors argue that "equity" entails causal judgments about which contributions count, and that observers vary in how they interpret causally ambiguous structural barriers. When information about gender-based structural barriers in a key performance indicator is salient in the decision context, observers with stronger structural beliefs (rigged worldview) view raw indicators as misrepresenting women's contributions and thus judge adjustments as fairer; observers with stronger individual beliefs (rewarding worldview) treat raw indicators as merit-aligned and judge adjustments as less fair. When information about structural barriers is absent, or when barriers are not gender-based, beliefs lose their moderating power. Fairness judgments in turn drive performance evaluations of the allocator.

**Theoretical Contribution**
The paper broadens the equity principle by showing that fairness judgments of equitable allocations depend on observers' beliefs in the causes of success and on contextualized information about structural barriers, rather than being a fixed weighting of raw performance indicators. It theoretically separates two related-but-distinct manifestations of meritocratic ideals (individual vs. structural causes of success) and demonstrates that structural beliefs play a more consistent moderating role than individual beliefs once contextual information is made salient — bridging organizational fairness research with social-justice and meritocracy scholarship.

**Practical Implication**
Organizational leaders pursuing DEI goals should provide salient, contextualized evidence of gender-based disparities in specific performance indicators rather than relying on equal-pay-for-equal-work framings; doing so leverages observers' structural beliefs and reduces pushback against adjustments. Framing barriers as objective features of the work environment (e.g., equipment design) is especially likely to broaden support, including from observers with weaker structural beliefs. Justifications consistent with observers' success beliefs may help managers anticipate and mitigate perceived unfairness around adjustments.

**Limitations**
The studies use vignette experiments rather than real organizational allocations, so external validity is limited; success beliefs were measured rather than experimentally manipulated, so causal inferences about their moderating role are constrained. All samples were drawn from the United States, where individual beliefs are stronger and structural beliefs more dispersed than elsewhere, and norms vary across cultures. Theorizing focused on gender, leaving intersectional and other-group barriers (e.g., race, LGBTQ+) unexamined; some measurement choices (preference for the merit principle scale, beliefs in causes of success) showed psychometric difficulties.

**Future Research**
Future work should validate and develop independent manipulations of individual versus structural beliefs, test the framework with field-level organizational allocation data, and examine whether the findings extend to recipients (not just observers) and to allocations of less-tangible, non-zero-sum resources such as respect. Researchers can extend the lens to other marginalized groups and intersectional identities, examine how exposure to structural barriers (e.g., reverse mentoring) shapes belief endorsement over time, and integrate the framework with affirmative-action research that has emphasized individual but not structural beliefs.

**APA 7th Citation**
Tedder-King, A., & Sherf, E. N. (2024). Fairness judgments in the context of structural sexism: The role of beliefs in individual and structural causes of success. *Academy of Management Journal*, 67(5), 1234-1272. https://doi.org/10.5465/amj.2022.0776
