---
id: amj-vol-68-no-2-fewer-2024
title: "CEO Political Partisanship and Corporate Misconduct"
authors:
  - "Fewer, T. J."
  - "Tarakci, M."
year: 2025
journal: "Academy of Management Journal"
doi: "https://doi.org/10.5465/amj.2022.0909"
volume: 68
issue: 2
pages: "357-379"

source: "AMJ/vol-68-no-2"
pdf_path: "library/AMJ/vol-68-no-2/pdfs/Fewer 2024 CEO Political Partisanship and Corporate Misconduct.pdf"
text_path: "library/AMJ/vol-68-no-2/text/Fewer 2024 CEO Political Partisanship and Corporate Misconduct.txt"
ingested_at: "2026-04-30"
extraction_model: "claude-opus-4-7"
extraction_version: "v2"

paper_type: "empirical-quantitative"
keywords: ["CEO political partisanship", "corporate misconduct", "political ideology", "ideology-as-identity", "ideology-as-values", "moral foundations", "perspective taking", "rationalization", "upper echelons", "Fortune 500"]
theory: ["upper echelons theory", "ideology-as-identity perspective", "ideology-as-values perspective", "social identity theory", "moral foundations theory", "rationalization (fraud triangle)"]
topics: ["ceo-leadership", "corporate-governance", "ethical-decision-making", "unethical-behavior"]
unit_of_analysis: "firm"
level_of_theory: "micro"
dependent_variable_family: "social"
methods: "Panel firm-year analysis of Fortune 500 firms 2010-2018; Poisson regressions of misconduct counts with one- and three-year lags, four-digit SIC industry and year fixed effects, and standard errors clustered by firm; CEO political ideology measured via the Chin et al. (2013) FEC-donations index, with partisanship operationalized as the squared deviation from the political center; difference-in-differences robustness using within-firm CEO turnovers (including exogenous deaths/illness); inverse-document-frequency text analysis of earnings-call Q&A speeches to probe perspective-taking and moral-foundations mechanisms."
sample:
  industry: "Cross-industry Fortune 500 firms (SIC industry fixed effects); regulatory enforcement actions span competition, consumer protection, employment, environmental, financial, government contracting, and health-and-safety violations from the Good Jobs First Violations Tracker"
  country: "United States"
  time_period: "2010-2018"
  units: "Firm-year observations (CEOs of Fortune 500 firms)"
  n: "498 firms, 831 CEOs, 3,950 firm-year observations"

evidence:
  sample_n: "the final sample comprises 498 firms"
  sample_country: "U.S. Federal Election Commission"
  sample_industry: "all firms listed in the Fortune 500"
  sample_time_period: "from 2010–2018"
  theories_overview: "ideology-as-identity as a key, understu"
  methods_overview: "we enlist a Poisson regression"
  keywords_source: "CEO political partisanship and corporate misconduct in a sample of Fortune 500 CEOs"
---

# CEO Political Partisanship and Corporate Misconduct

**Abstract**
Are firms led by liberal versus conservative CEOs more likely to engage in corporate misconduct? This study unearths the ideological bases of misconduct by distinguishing the values and identity perspectives of political ideology. Rather than attributing misconduct to liberal or conservative values, we introduce and examine CEO political partisanship—that is, the strength of a CEO's identification with political and ideological groups. We hypothesize and find robust evidence for a positive relationship between CEO political partisanship and corporate misconduct in a sample of Fortune 500 CEOs from 2010 to 2018. Our findings thus contribute to the conversation on the role of political ideology in organizations by unearthing the organizational implications of political identities. As CEOs increasingly engage in political discussions and political divisions grow stronger, our study offers a timely warning about the harmful link between CEO political partisanship and corporate misconduct.

**Research Question**
Are firms led by politically partisan CEOs (strongly liberal or strongly conservative) more likely to engage in corporate misconduct than firms led by politically nonpartisan (moderate) CEOs, and does the explanatory power lie in the identity dimension of political ideology rather than its values dimension?

**Mechanism Process**
- IV(s): CEO political partisanship (squared deviation of Chin et al. 2013 FEC-donations ideology score from the political center); CEO political ideology (linear left-right value)
- DV(s): Corporate misconduct (count of regulatory enforcement actions from the Good Jobs First Violations Tracker, with one-year and three-year lags)
- Mediators (theorized): reduced perspective taking; elevated moral foundations (proxied via inverse-document-frequency analysis of CEO speech in quarterly earnings-call Q&A)
- Moderators / controls: CEO age, gender, narcissism (Chatterjee–Hambrick index), market value of stock, tenure, duality; firm cash, dividends, capital expenditures, debt ratio, market capitalization, net income, share-price performance; board size, board insiders; four-digit SIC industry and year fixed effects

Drawing on the ideology-as-identity perspective, the authors argue that strong identification with a political group—not the content of liberal or conservative values—predisposes CEOs to rationalize corporate misconduct. Partisan CEOs (a) take the perspectives of out-group stakeholders less, narrowing concern for those harmed by wrongdoing, and (b) experience elevated moral foundations that grant them a sense of moral superiority, freeing them from a felt obligation to police firm conduct or framing transgressions as righteous retribution against "immoral" outsiders. The empirical analysis fails to detect a perspective-taking link to misconduct but supports the moral-foundations mechanism: partisan CEOs use more moral language in earnings calls, and elevated moral language is positively associated with misconduct.

**Theoretical Contribution**
The paper advances CEO-based theories of corporate misconduct by introducing political ideology as an antecedent and, more pointedly, by isolating the previously understudied identity dimension (partisanship) from the values dimension. It contributes to research on political ideology in organizations by showing that political identity—not liberal vs. conservative values—drives a non-political organizational outcome (regulatory wrongdoing), thereby integrating an "ideology-as-identity" lens into upper echelons research. It also contributes to the political-science/organization-studies nexus by demonstrating moral foundations as the cognitive bridge through which partisan identities translate into harmful firm behavior.

**Practical Implication**
Boards, regulators, and stakeholders should treat the strength of a CEO's political identification—not just its left-right direction—as a risk factor for corporate misconduct, since strongly partisan CEOs of either ideology are more likely to preside over enforcement-triggering wrongdoing. Governance designs that increase ideological diversity on top management teams and boards, and monitoring practices that resist deference to a CEO's self-perceived moral authority, may attenuate the partisanship-misconduct link. As executives engage more publicly in political discourse and polarization deepens, attention to CEOs' partisan identities offers a timely warning signal for organizational risk.

**Limitations**
The study cannot disentangle whether tracked-but-unobserved value differences predict difficult-to-prosecute forms of misconduct that fall outside the Violations Tracker's scope, and the violations-based proxy depends on detection. Political donations are an unobtrusive but indirect ideology proxy and may also reflect strategic favor-seeking from regulators. The findings come from a U.S. bipartisan Fortune 500 setting, limiting generalization to multiparty or authoritarian contexts and to non-elite organizations. Despite difference-in-differences and exogenous-turnover robustness checks, residual omitted-variable concerns remain.

**Future Research**
Future work should test whether other deep-rooted belief systems (religious, populist) drive misconduct, and how political ideology interacts with pressure and opportunity (e.g., board ideology, stakeholder alignment) to shape misconduct. Researchers should pursue more direct ideology measures (e.g., social-media content analysis, factor-analytic separation of values and identity), dyadic and longer-run mechanism tests of perspective taking and moral foundations, and cross-national designs in multiparty or authoritarian regimes. Studies should also examine whether partisan CEOs alienate ideologically distant stakeholders and whether ideologically homogeneous TMTs and boards function as echo chambers that magnify the partisanship-misconduct effect.

**APA 7th Citation**
Fewer, T. J., & Tarakci, M. (2025). CEO political partisanship and corporate misconduct. *Academy of Management Journal*, 68(2), 357-379. https://doi.org/10.5465/amj.2022.0909
